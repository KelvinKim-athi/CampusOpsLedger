...
class FinanceClearanceControlRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceClearanceControlRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceClearanceControlEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceClearanceControlEvidence":
        return cls(
...
@dataclass(frozen=True)
class FinanceClearanceControlEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceClearanceControlEvent":
        return cls(
...
@dataclass(frozen=True)
class FinanceClearanceControlRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[FinanceClearanceControlEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceClearanceControlEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceClearanceControlRule]) -> "FinanceClearanceControlRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceClearanceControlRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceClearanceControlRecord":
        status_key = clean_status(status)
        event = FinanceClearanceControlEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceClearanceControlRecord":
    ...
    def add_evidence(self, evidence: FinanceClearanceControlEvidence) -> "FinanceClearanceControlRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceClearanceControlRecord":
        return cls(
            ...
            evidence=tuple(FinanceClearanceControlEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceClearanceControlEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceClearanceControlRegister:
    domain = "finance_clearance_control"

    def __init__(
        self,
        records: Iterable[FinanceClearanceControlRecord] | None = None,
        rules: Iterable[FinanceClearanceControlRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceClearanceControlRecord] = {}
        self._rules: dict[str, FinanceClearanceControlRule] = {}
    ...
    def add_rule(self, rule: FinanceClearanceControlRule) -> FinanceClearanceControlRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceClearanceControlRecord, *, evaluate: bool = True) -> FinanceClearanceControlRecord:
    ...
    def get(self, record_id: object) -> FinanceClearanceControlRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceClearanceControlRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceClearanceControlRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceClearanceControlEvidence) -> FinanceClearanceControlRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceClearanceControlRecord]:
        updated: list[FinanceClearanceControlRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceClearanceControlRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceClearanceControlRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceClearanceControlRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceClearanceControlRule, ...]:
    return (
        FinanceClearanceControlRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceClearanceControlRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceClearanceControlRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceClearanceControlRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_clearance_control")
    return FinanceClearanceControlRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_clearance_control",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_clearance_control",),
    )


def demo_register() -> FinanceClearanceControlRegister:
    register = FinanceClearanceControlRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_clearance_control record", "campus.office", "1250", priority="urgent"))
    return register
