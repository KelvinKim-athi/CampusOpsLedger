...
class StudentSlaWatchRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentSlaWatchRule":
        return cls(
...
class StudentSlaWatchEvidence:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentSlaWatchEvidence":
        return cls(
...
class StudentSlaWatchEvent:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentSlaWatchEvent":
        return cls(
...
class StudentSlaWatchRecord:
    ...
    evidence: tuple[StudentSlaWatchEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentSlaWatchEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentSlaWatchRule]) -> "StudentSlaWatchRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentSlaWatchRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentSlaWatchRecord":
        status_key = clean_status(status)
        event = StudentSlaWatchEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentSlaWatchRecord":
    ...
    def add_evidence(self, evidence: StudentSlaWatchEvidence) -> "StudentSlaWatchRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentSlaWatchRecord":
        return cls(
            ...
            evidence=tuple(StudentSlaWatchEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentSlaWatchEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentSlaWatchRegister:
    domain = "student_sla_watch"

    def __init__(
        self,
        records: Iterable[StudentSlaWatchRecord] | None = None,
        rules: Iterable[StudentSlaWatchRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentSlaWatchRecord] = {}
        self._rules: dict[str, StudentSlaWatchRule] = {}
    ...
    def add_rule(self, rule: StudentSlaWatchRule) -> StudentSlaWatchRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentSlaWatchRecord, *, evaluate: bool = True) -> StudentSlaWatchRecord:
    ...
    def get(self, record_id: object) -> StudentSlaWatchRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentSlaWatchRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentSlaWatchRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentSlaWatchEvidence) -> StudentSlaWatchRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentSlaWatchRecord]:
        updated: list[StudentSlaWatchRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentSlaWatchRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentSlaWatchRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentSlaWatchRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentSlaWatchRule, ...]:
    return (
        StudentSlaWatchRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentSlaWatchRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentSlaWatchRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentSlaWatchRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_sla_watch")
    return StudentSlaWatchRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_sla_watch",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_sla_watch",),
    )


def demo_register() -> StudentSlaWatchRegister:
    register = StudentSlaWatchRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_sla_watch record", "campus.office", "1250", priority="urgent"))
    return register
