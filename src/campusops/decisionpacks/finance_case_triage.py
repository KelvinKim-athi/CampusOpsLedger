...
class FinanceCaseTriageRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceCaseTriageRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceCaseTriageEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceCaseTriageEvidence":
        return cls(
...
class FinanceCaseTriageEvent:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceCaseTriageEvent":
        return cls(
...
class FinanceCaseTriageRecord:
    ...
    evidence: tuple[FinanceCaseTriageEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceCaseTriageEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceCaseTriageRule]) -> "FinanceCaseTriageRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceCaseTriageRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceCaseTriageRecord":
        status_key = clean_status(status)
        event = FinanceCaseTriageEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceCaseTriageRecord":
    ...
    def add_evidence(self, evidence: FinanceCaseTriageEvidence) -> "FinanceCaseTriageRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceCaseTriageRecord":
        return cls(
            ...
            evidence=tuple(FinanceCaseTriageEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceCaseTriageEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceCaseTriageRegister:
    domain = "finance_case_triage"

    def __init__(
        self,
        records: Iterable[FinanceCaseTriageRecord] | None = None,
        rules: Iterable[FinanceCaseTriageRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceCaseTriageRecord] = {}
        self._rules: dict[str, FinanceCaseTriageRule] = {}
    ...
    def add_rule(self, rule: FinanceCaseTriageRule) -> FinanceCaseTriageRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceCaseTriageRecord, *, evaluate: bool = True) -> FinanceCaseTriageRecord:
    ...
    def get(self, record_id: object) -> FinanceCaseTriageRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceCaseTriageRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceCaseTriageRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceCaseTriageEvidence) -> FinanceCaseTriageRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceCaseTriageRecord]:
        updated: list[FinanceCaseTriageRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceCaseTriageRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceCaseTriageRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceCaseTriageRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceCaseTriageRule, ...]:
    return (
        FinanceCaseTriageRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceCaseTriageRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceCaseTriageRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceCaseTriageRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_case_triage")
    return FinanceCaseTriageRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_case_triage",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_case_triage",),
    )


def demo_register() -> FinanceCaseTriageRegister:
    register = FinanceCaseTriageRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_case_triage record", "campus.office", "1250", priority="urgent"))
    return register
