...
class StudentDeadlineMonitoringRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentDeadlineMonitoringRule":
        return cls(
...
@dataclass(frozen=True)
class StudentDeadlineMonitoringEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentDeadlineMonitoringEvidence":
        return cls(
...
@dataclass(frozen=True)
class StudentDeadlineMonitoringEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentDeadlineMonitoringEvent":
        return cls(
...
@dataclass(frozen=True)
class StudentDeadlineMonitoringRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[StudentDeadlineMonitoringEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentDeadlineMonitoringEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentDeadlineMonitoringRule]) -> "StudentDeadlineMonitoringRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentDeadlineMonitoringRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentDeadlineMonitoringRecord":
        status_key = clean_status(status)
        event = StudentDeadlineMonitoringEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentDeadlineMonitoringRecord":
    ...
    def add_evidence(self, evidence: StudentDeadlineMonitoringEvidence) -> "StudentDeadlineMonitoringRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentDeadlineMonitoringRecord":
        return cls(
            ...
            evidence=tuple(StudentDeadlineMonitoringEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentDeadlineMonitoringEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentDeadlineMonitoringRegister:
    domain = "student_deadline_monitoring"

    def __init__(
        self,
        records: Iterable[StudentDeadlineMonitoringRecord] | None = None,
        rules: Iterable[StudentDeadlineMonitoringRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentDeadlineMonitoringRecord] = {}
        self._rules: dict[str, StudentDeadlineMonitoringRule] = {}
    ...
    def add_rule(self, rule: StudentDeadlineMonitoringRule) -> StudentDeadlineMonitoringRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentDeadlineMonitoringRecord, *, evaluate: bool = True) -> StudentDeadlineMonitoringRecord:
    ...
    def get(self, record_id: object) -> StudentDeadlineMonitoringRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentDeadlineMonitoringRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentDeadlineMonitoringRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentDeadlineMonitoringEvidence) -> StudentDeadlineMonitoringRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentDeadlineMonitoringRecord]:
        updated: list[StudentDeadlineMonitoringRecord] = []
    ...
    def all_records(self) -> list[StudentDeadlineMonitoringRecord]:
        return sorted(self._records.values(), key=lambda item: item.record_id)

    def open_records(self) -> list[StudentDeadlineMonitoringRecord]:
        return [record for record in self.all_records() if record.is_open()]

    def final_records(self) -> list[StudentDeadlineMonitoringRecord]:
        return [record for record in self.all_records() if not record.is_open()]

    def by_owner(self, owner: object) -> list[StudentDeadlineMonitoringRecord]:
        wanted = clean_key(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def by_status(self, status: object) -> list[StudentDeadlineMonitoringRecord]:
        wanted = clean_status(status)
        return [record for record in self.all_records() if record.status == wanted]

    def by_category(self, category: object) -> list[StudentDeadlineMonitoringRecord]:
        wanted = clean_key(category)
        return [record for record in self.all_records() if record.category == wanted]

    def by_tag(self, tag: object) -> list[StudentDeadlineMonitoringRecord]:
        wanted = clean_key(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def due_records(self, at: object | None = None) -> list[StudentDeadlineMonitoringRecord]:
        return [record for record in self.all_records() if record.is_due(at)]

    def high_risk_records(self, minimum_score: int = 40, *, at: object | None = None) -> list[StudentDeadlineMonitoringRecord]:
        return sorted(
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentDeadlineMonitoringRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentDeadlineMonitoringRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentDeadlineMonitoringRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentDeadlineMonitoringRule, ...]:
    return (
        StudentDeadlineMonitoringRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentDeadlineMonitoringRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentDeadlineMonitoringRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentDeadlineMonitoringRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_deadline_monitoring")
    return StudentDeadlineMonitoringRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_deadline_monitoring",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_deadline_monitoring",),
    )


def demo_register() -> StudentDeadlineMonitoringRegister:
    register = StudentDeadlineMonitoringRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_deadline_monitoring record", "campus.office", "1250", priority="urgent"))
    return register
