...
class FinanceDeadlineMonitoringRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceDeadlineMonitoringRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceDeadlineMonitoringEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceDeadlineMonitoringEvidence":
        return cls(
...
@dataclass(frozen=True)
class FinanceDeadlineMonitoringEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceDeadlineMonitoringEvent":
        return cls(
...
@dataclass(frozen=True)
class FinanceDeadlineMonitoringRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[FinanceDeadlineMonitoringEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceDeadlineMonitoringEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceDeadlineMonitoringRule]) -> "FinanceDeadlineMonitoringRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceDeadlineMonitoringRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceDeadlineMonitoringRecord":
        status_key = clean_status(status)
        event = FinanceDeadlineMonitoringEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceDeadlineMonitoringRecord":
    ...
    def add_evidence(self, evidence: FinanceDeadlineMonitoringEvidence) -> "FinanceDeadlineMonitoringRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceDeadlineMonitoringRecord":
        return cls(
            ...
            evidence=tuple(FinanceDeadlineMonitoringEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceDeadlineMonitoringEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceDeadlineMonitoringRegister:
    domain = "finance_deadline_monitoring"

    def __init__(
        self,
        records: Iterable[FinanceDeadlineMonitoringRecord] | None = None,
        rules: Iterable[FinanceDeadlineMonitoringRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceDeadlineMonitoringRecord] = {}
        self._rules: dict[str, FinanceDeadlineMonitoringRule] = {}
    ...
    def add_rule(self, rule: FinanceDeadlineMonitoringRule) -> FinanceDeadlineMonitoringRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceDeadlineMonitoringRecord, *, evaluate: bool = True) -> FinanceDeadlineMonitoringRecord:
    ...
    def get(self, record_id: object) -> FinanceDeadlineMonitoringRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceDeadlineMonitoringRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceDeadlineMonitoringRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceDeadlineMonitoringEvidence) -> FinanceDeadlineMonitoringRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceDeadlineMonitoringRecord]:
        updated: list[FinanceDeadlineMonitoringRecord] = []
    ...
    def all_records(self) -> list[FinanceDeadlineMonitoringRecord]:
        return sorted(self._records.values(), key=lambda item: item.record_id)

    def open_records(self) -> list[FinanceDeadlineMonitoringRecord]:
        return [record for record in self.all_records() if record.is_open()]

    def final_records(self) -> list[FinanceDeadlineMonitoringRecord]:
        return [record for record in self.all_records() if not record.is_open()]

    def by_owner(self, owner: object) -> list[FinanceDeadlineMonitoringRecord]:
        wanted = clean_key(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def by_status(self, status: object) -> list[FinanceDeadlineMonitoringRecord]:
        wanted = clean_status(status)
        return [record for record in self.all_records() if record.status == wanted]

    def by_category(self, category: object) -> list[FinanceDeadlineMonitoringRecord]:
        wanted = clean_key(category)
        return [record for record in self.all_records() if record.category == wanted]

    def by_tag(self, tag: object) -> list[FinanceDeadlineMonitoringRecord]:
        wanted = clean_key(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def due_records(self, at: object | None = None) -> list[FinanceDeadlineMonitoringRecord]:
        return [record for record in self.all_records() if record.is_due(at)]

    def high_risk_records(self, minimum_score: int = 40, *, at: object | None = None) -> list[FinanceDeadlineMonitoringRecord]:
        return sorted(
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceDeadlineMonitoringRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceDeadlineMonitoringRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceDeadlineMonitoringRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceDeadlineMonitoringRule, ...]:
    return (
        FinanceDeadlineMonitoringRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceDeadlineMonitoringRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceDeadlineMonitoringRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceDeadlineMonitoringRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_deadline_monitoring")
    return FinanceDeadlineMonitoringRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_deadline_monitoring",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_deadline_monitoring",),
    )


def demo_register() -> FinanceDeadlineMonitoringRegister:
    register = FinanceDeadlineMonitoringRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_deadline_monitoring record", "campus.office", "1250", priority="urgent"))
    return register
