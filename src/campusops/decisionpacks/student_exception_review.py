...
class StudentExceptionReviewRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentExceptionReviewRule":
        return cls(
...
@dataclass(frozen=True)
class StudentExceptionReviewEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentExceptionReviewEvidence":
        return cls(
...
@dataclass(frozen=True)
class StudentExceptionReviewEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentExceptionReviewEvent":
        return cls(
...
@dataclass(frozen=True)
class StudentExceptionReviewRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[StudentExceptionReviewEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentExceptionReviewEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentExceptionReviewRule]) -> "StudentExceptionReviewRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentExceptionReviewRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentExceptionReviewRecord":
        status_key = clean_status(status)
        event = StudentExceptionReviewEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentExceptionReviewRecord":
    ...
    def add_evidence(self, evidence: StudentExceptionReviewEvidence) -> "StudentExceptionReviewRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentExceptionReviewRecord":
        return cls(
            ...
            evidence=tuple(StudentExceptionReviewEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentExceptionReviewEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentExceptionReviewRegister:
    domain = "student_exception_review"

    def __init__(
        self,
        records: Iterable[StudentExceptionReviewRecord] | None = None,
        rules: Iterable[StudentExceptionReviewRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentExceptionReviewRecord] = {}
        self._rules: dict[str, StudentExceptionReviewRule] = {}
    ...
    def add_rule(self, rule: StudentExceptionReviewRule) -> StudentExceptionReviewRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentExceptionReviewRecord, *, evaluate: bool = True) -> StudentExceptionReviewRecord:
    ...
    def get(self, record_id: object) -> StudentExceptionReviewRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentExceptionReviewRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentExceptionReviewRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentExceptionReviewEvidence) -> StudentExceptionReviewRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentExceptionReviewRecord]:
        updated: list[StudentExceptionReviewRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentExceptionReviewRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentExceptionReviewRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentExceptionReviewRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentExceptionReviewRule, ...]:
    return (
        StudentExceptionReviewRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentExceptionReviewRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentExceptionReviewRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentExceptionReviewRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_exception_review")
    return StudentExceptionReviewRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_exception_review",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_exception_review",),
    )


def demo_register() -> StudentExceptionReviewRegister:
    register = StudentExceptionReviewRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_exception_review record", "campus.office", "1250", priority="urgent"))
    return register
