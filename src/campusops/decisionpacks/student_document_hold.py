...
class StudentDocumentHoldRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentDocumentHoldRule":
        return cls(
...
@dataclass(frozen=True)
class StudentDocumentHoldEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentDocumentHoldEvidence":
        return cls(
...
@dataclass(frozen=True)
class StudentDocumentHoldEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentDocumentHoldEvent":
        return cls(
...
@dataclass(frozen=True)
class StudentDocumentHoldRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[StudentDocumentHoldEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentDocumentHoldEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentDocumentHoldRule]) -> "StudentDocumentHoldRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentDocumentHoldRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentDocumentHoldRecord":
        status_key = clean_status(status)
        event = StudentDocumentHoldEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentDocumentHoldRecord":
    ...
    def add_evidence(self, evidence: StudentDocumentHoldEvidence) -> "StudentDocumentHoldRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentDocumentHoldRecord":
        return cls(
            ...
            evidence=tuple(StudentDocumentHoldEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentDocumentHoldEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentDocumentHoldRegister:
    domain = "student_document_hold"

    def __init__(
        self,
        records: Iterable[StudentDocumentHoldRecord] | None = None,
        rules: Iterable[StudentDocumentHoldRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentDocumentHoldRecord] = {}
        self._rules: dict[str, StudentDocumentHoldRule] = {}
    ...
    def add_rule(self, rule: StudentDocumentHoldRule) -> StudentDocumentHoldRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentDocumentHoldRecord, *, evaluate: bool = True) -> StudentDocumentHoldRecord:
    ...
    def get(self, record_id: object) -> StudentDocumentHoldRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentDocumentHoldRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentDocumentHoldRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentDocumentHoldEvidence) -> StudentDocumentHoldRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentDocumentHoldRecord]:
        updated: list[StudentDocumentHoldRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentDocumentHoldRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentDocumentHoldRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentDocumentHoldRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentDocumentHoldRule, ...]:
    return (
        StudentDocumentHoldRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentDocumentHoldRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentDocumentHoldRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentDocumentHoldRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_document_hold")
    return StudentDocumentHoldRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_document_hold",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_document_hold",),
    )


def demo_register() -> StudentDocumentHoldRegister:
    register = StudentDocumentHoldRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_document_hold record", "campus.office", "1250", priority="urgent"))
    return register
