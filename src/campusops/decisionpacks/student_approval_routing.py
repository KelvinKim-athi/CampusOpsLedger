...
class StudentApprovalRoutingRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentApprovalRoutingRule":
        return cls(
...
@dataclass(frozen=True)
class StudentApprovalRoutingEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentApprovalRoutingEvidence":
        return cls(
...
@dataclass(frozen=True)
class StudentApprovalRoutingEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentApprovalRoutingEvent":
        return cls(
...
@dataclass(frozen=True)
class StudentApprovalRoutingRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[StudentApprovalRoutingEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentApprovalRoutingEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentApprovalRoutingRule]) -> "StudentApprovalRoutingRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentApprovalRoutingRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentApprovalRoutingRecord":
        status_key = clean_status(status)
        event = StudentApprovalRoutingEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentApprovalRoutingRecord":
    ...
    def add_evidence(self, evidence: StudentApprovalRoutingEvidence) -> "StudentApprovalRoutingRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentApprovalRoutingRecord":
        return cls(
            ...
            evidence=tuple(StudentApprovalRoutingEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentApprovalRoutingEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentApprovalRoutingRegister:
    domain = "student_approval_routing"

    def __init__(
        self,
        records: Iterable[StudentApprovalRoutingRecord] | None = None,
        rules: Iterable[StudentApprovalRoutingRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentApprovalRoutingRecord] = {}
        self._rules: dict[str, StudentApprovalRoutingRule] = {}
    ...
    def add_rule(self, rule: StudentApprovalRoutingRule) -> StudentApprovalRoutingRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentApprovalRoutingRecord, *, evaluate: bool = True) -> StudentApprovalRoutingRecord:
    ...
    def get(self, record_id: object) -> StudentApprovalRoutingRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentApprovalRoutingRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentApprovalRoutingRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentApprovalRoutingEvidence) -> StudentApprovalRoutingRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentApprovalRoutingRecord]:
        updated: list[StudentApprovalRoutingRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentApprovalRoutingRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentApprovalRoutingRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentApprovalRoutingRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentApprovalRoutingRule, ...]:
    return (
        StudentApprovalRoutingRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentApprovalRoutingRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentApprovalRoutingRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentApprovalRoutingRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_approval_routing")
    return StudentApprovalRoutingRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_approval_routing",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_approval_routing",),
    )


def demo_register() -> StudentApprovalRoutingRegister:
    register = StudentApprovalRoutingRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_approval_routing record", "campus.office", "1250", priority="urgent"))
    return register
