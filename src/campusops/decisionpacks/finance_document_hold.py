...
class FinanceDocumentHoldRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceDocumentHoldRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceDocumentHoldEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceDocumentHoldEvidence":
        return cls(
...
@dataclass(frozen=True)
class FinanceDocumentHoldEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceDocumentHoldEvent":
        return cls(
...
@dataclass(frozen=True)
class FinanceDocumentHoldRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[FinanceDocumentHoldEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceDocumentHoldEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceDocumentHoldRule]) -> "FinanceDocumentHoldRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceDocumentHoldRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceDocumentHoldRecord":
        status_key = clean_status(status)
        event = FinanceDocumentHoldEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceDocumentHoldRecord":
    ...
    def add_evidence(self, evidence: FinanceDocumentHoldEvidence) -> "FinanceDocumentHoldRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceDocumentHoldRecord":
        return cls(
            ...
            evidence=tuple(FinanceDocumentHoldEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceDocumentHoldEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceDocumentHoldRegister:
    domain = "finance_document_hold"

    def __init__(
        self,
        records: Iterable[FinanceDocumentHoldRecord] | None = None,
        rules: Iterable[FinanceDocumentHoldRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceDocumentHoldRecord] = {}
        self._rules: dict[str, FinanceDocumentHoldRule] = {}
    ...
    def add_rule(self, rule: FinanceDocumentHoldRule) -> FinanceDocumentHoldRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceDocumentHoldRecord, *, evaluate: bool = True) -> FinanceDocumentHoldRecord:
    ...
    def get(self, record_id: object) -> FinanceDocumentHoldRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceDocumentHoldRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceDocumentHoldRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceDocumentHoldEvidence) -> FinanceDocumentHoldRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceDocumentHoldRecord]:
        updated: list[FinanceDocumentHoldRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceDocumentHoldRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceDocumentHoldRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceDocumentHoldRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceDocumentHoldRule, ...]:
    return (
        FinanceDocumentHoldRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceDocumentHoldRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceDocumentHoldRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceDocumentHoldRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_document_hold")
    return FinanceDocumentHoldRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_document_hold",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_document_hold",),
    )


def demo_register() -> FinanceDocumentHoldRegister:
    register = FinanceDocumentHoldRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_document_hold record", "campus.office", "1250", priority="urgent"))
    return register
