...
class FinanceApprovalRoutingRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceApprovalRoutingRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceApprovalRoutingEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceApprovalRoutingEvidence":
        return cls(
...
@dataclass(frozen=True)
class FinanceApprovalRoutingEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceApprovalRoutingEvent":
        return cls(
...
@dataclass(frozen=True)
class FinanceApprovalRoutingRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[FinanceApprovalRoutingEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceApprovalRoutingEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceApprovalRoutingRule]) -> "FinanceApprovalRoutingRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceApprovalRoutingRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceApprovalRoutingRecord":
        status_key = clean_status(status)
        event = FinanceApprovalRoutingEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceApprovalRoutingRecord":
    ...
    def add_evidence(self, evidence: FinanceApprovalRoutingEvidence) -> "FinanceApprovalRoutingRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceApprovalRoutingRecord":
        return cls(
            ...
            evidence=tuple(FinanceApprovalRoutingEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceApprovalRoutingEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceApprovalRoutingRegister:
    domain = "finance_approval_routing"

    def __init__(
        self,
        records: Iterable[FinanceApprovalRoutingRecord] | None = None,
        rules: Iterable[FinanceApprovalRoutingRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceApprovalRoutingRecord] = {}
        self._rules: dict[str, FinanceApprovalRoutingRule] = {}
    ...
    def add_rule(self, rule: FinanceApprovalRoutingRule) -> FinanceApprovalRoutingRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceApprovalRoutingRecord, *, evaluate: bool = True) -> FinanceApprovalRoutingRecord:
    ...
    def get(self, record_id: object) -> FinanceApprovalRoutingRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceApprovalRoutingRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceApprovalRoutingRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceApprovalRoutingEvidence) -> FinanceApprovalRoutingRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceApprovalRoutingRecord]:
        updated: list[FinanceApprovalRoutingRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceApprovalRoutingRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceApprovalRoutingRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceApprovalRoutingRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceApprovalRoutingRule, ...]:
    return (
        FinanceApprovalRoutingRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceApprovalRoutingRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceApprovalRoutingRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceApprovalRoutingRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_approval_routing")
    return FinanceApprovalRoutingRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_approval_routing",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_approval_routing",),
    )


def demo_register() -> FinanceApprovalRoutingRegister:
    register = FinanceApprovalRoutingRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_approval_routing record", "campus.office", "1250", priority="urgent"))
    return register
