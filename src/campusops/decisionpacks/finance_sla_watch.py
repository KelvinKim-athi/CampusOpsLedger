...
class FinanceSlaWatchRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceSlaWatchRule":
        return cls(
...
class FinanceSlaWatchEvidence:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceSlaWatchEvidence":
        return cls(
...
class FinanceSlaWatchEvent:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceSlaWatchEvent":
        return cls(
...
class FinanceSlaWatchRecord:
    ...
    evidence: tuple[FinanceSlaWatchEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceSlaWatchEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceSlaWatchRule]) -> "FinanceSlaWatchRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceSlaWatchRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceSlaWatchRecord":
        status_key = clean_status(status)
        event = FinanceSlaWatchEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceSlaWatchRecord":
    ...
    def add_evidence(self, evidence: FinanceSlaWatchEvidence) -> "FinanceSlaWatchRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceSlaWatchRecord":
        return cls(
            ...
            evidence=tuple(FinanceSlaWatchEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceSlaWatchEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceSlaWatchRegister:
    domain = "finance_sla_watch"

    def __init__(
        self,
        records: Iterable[FinanceSlaWatchRecord] | None = None,
        rules: Iterable[FinanceSlaWatchRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceSlaWatchRecord] = {}
        self._rules: dict[str, FinanceSlaWatchRule] = {}
    ...
    def add_rule(self, rule: FinanceSlaWatchRule) -> FinanceSlaWatchRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceSlaWatchRecord, *, evaluate: bool = True) -> FinanceSlaWatchRecord:
    ...
    def get(self, record_id: object) -> FinanceSlaWatchRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceSlaWatchRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceSlaWatchRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceSlaWatchEvidence) -> FinanceSlaWatchRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceSlaWatchRecord]:
        updated: list[FinanceSlaWatchRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceSlaWatchRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceSlaWatchRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceSlaWatchRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceSlaWatchRule, ...]:
    return (
        FinanceSlaWatchRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceSlaWatchRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceSlaWatchRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceSlaWatchRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_sla_watch")
    return FinanceSlaWatchRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_sla_watch",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_sla_watch",),
    )


def demo_register() -> FinanceSlaWatchRegister:
    register = FinanceSlaWatchRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_sla_watch record", "campus.office", "1250", priority="urgent"))
    return register
