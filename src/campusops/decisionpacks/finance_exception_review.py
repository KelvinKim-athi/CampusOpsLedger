...
class FinanceExceptionReviewRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceExceptionReviewRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceExceptionReviewEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceExceptionReviewEvidence":
        return cls(
...
@dataclass(frozen=True)
class FinanceExceptionReviewEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceExceptionReviewEvent":
        return cls(
...
@dataclass(frozen=True)
class FinanceExceptionReviewRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[FinanceExceptionReviewEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceExceptionReviewEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceExceptionReviewRule]) -> "FinanceExceptionReviewRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceExceptionReviewRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceExceptionReviewRecord":
        status_key = clean_status(status)
        event = FinanceExceptionReviewEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceExceptionReviewRecord":
    ...
    def add_evidence(self, evidence: FinanceExceptionReviewEvidence) -> "FinanceExceptionReviewRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceExceptionReviewRecord":
        return cls(
            ...
            evidence=tuple(FinanceExceptionReviewEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceExceptionReviewEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceExceptionReviewRegister:
    domain = "finance_exception_review"

    def __init__(
        self,
        records: Iterable[FinanceExceptionReviewRecord] | None = None,
        rules: Iterable[FinanceExceptionReviewRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceExceptionReviewRecord] = {}
        self._rules: dict[str, FinanceExceptionReviewRule] = {}
    ...
    def add_rule(self, rule: FinanceExceptionReviewRule) -> FinanceExceptionReviewRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceExceptionReviewRecord, *, evaluate: bool = True) -> FinanceExceptionReviewRecord:
    ...
    def get(self, record_id: object) -> FinanceExceptionReviewRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceExceptionReviewRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceExceptionReviewRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceExceptionReviewEvidence) -> FinanceExceptionReviewRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceExceptionReviewRecord]:
        updated: list[FinanceExceptionReviewRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceExceptionReviewRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceExceptionReviewRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceExceptionReviewRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceExceptionReviewRule, ...]:
    return (
        FinanceExceptionReviewRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceExceptionReviewRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceExceptionReviewRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceExceptionReviewRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_exception_review")
    return FinanceExceptionReviewRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_exception_review",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_exception_review",),
    )


def demo_register() -> FinanceExceptionReviewRegister:
    register = FinanceExceptionReviewRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_exception_review record", "campus.office", "1250", priority="urgent"))
    return register
