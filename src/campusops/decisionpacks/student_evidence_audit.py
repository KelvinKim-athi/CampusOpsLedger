...
class StudentEvidenceAuditRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentEvidenceAuditRule":
        return cls(
...
@dataclass(frozen=True)
class StudentEvidenceAuditEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentEvidenceAuditEvidence":
        return cls(
...
@dataclass(frozen=True)
class StudentEvidenceAuditEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentEvidenceAuditEvent":
        return cls(
...
@dataclass(frozen=True)
class StudentEvidenceAuditRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[StudentEvidenceAuditEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentEvidenceAuditEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentEvidenceAuditRule]) -> "StudentEvidenceAuditRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentEvidenceAuditRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentEvidenceAuditRecord":
        status_key = clean_status(status)
        event = StudentEvidenceAuditEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentEvidenceAuditRecord":
    ...
    def add_evidence(self, evidence: StudentEvidenceAuditEvidence) -> "StudentEvidenceAuditRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentEvidenceAuditRecord":
        return cls(
            ...
            evidence=tuple(StudentEvidenceAuditEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentEvidenceAuditEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentEvidenceAuditRegister:
    domain = "student_evidence_audit"

    def __init__(
        self,
        records: Iterable[StudentEvidenceAuditRecord] | None = None,
        rules: Iterable[StudentEvidenceAuditRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentEvidenceAuditRecord] = {}
        self._rules: dict[str, StudentEvidenceAuditRule] = {}
    ...
    def add_rule(self, rule: StudentEvidenceAuditRule) -> StudentEvidenceAuditRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentEvidenceAuditRecord, *, evaluate: bool = True) -> StudentEvidenceAuditRecord:
    ...
    def get(self, record_id: object) -> StudentEvidenceAuditRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentEvidenceAuditRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentEvidenceAuditRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentEvidenceAuditEvidence) -> StudentEvidenceAuditRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentEvidenceAuditRecord]:
        updated: list[StudentEvidenceAuditRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentEvidenceAuditRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentEvidenceAuditRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentEvidenceAuditRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentEvidenceAuditRule, ...]:
    return (
        StudentEvidenceAuditRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentEvidenceAuditRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentEvidenceAuditRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentEvidenceAuditRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_evidence_audit")
    return StudentEvidenceAuditRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_evidence_audit",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_evidence_audit",),
    )


def demo_register() -> StudentEvidenceAuditRegister:
    register = StudentEvidenceAuditRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_evidence_audit record", "campus.office", "1250", priority="urgent"))
    return register
