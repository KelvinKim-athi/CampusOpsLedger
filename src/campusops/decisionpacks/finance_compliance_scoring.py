...
class FinanceComplianceScoringRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceComplianceScoringRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceComplianceScoringEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceComplianceScoringEvidence":
        return cls(
...
@dataclass(frozen=True)
class FinanceComplianceScoringEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceComplianceScoringEvent":
        return cls(
...
@dataclass(frozen=True)
class FinanceComplianceScoringRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[FinanceComplianceScoringEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceComplianceScoringEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceComplianceScoringRule]) -> "FinanceComplianceScoringRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceComplianceScoringRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceComplianceScoringRecord":
        status_key = clean_status(status)
        event = FinanceComplianceScoringEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceComplianceScoringRecord":
    ...
    def add_evidence(self, evidence: FinanceComplianceScoringEvidence) -> "FinanceComplianceScoringRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceComplianceScoringRecord":
        return cls(
            ...
            evidence=tuple(FinanceComplianceScoringEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceComplianceScoringEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceComplianceScoringRegister:
    domain = "finance_compliance_scoring"

    def __init__(
        self,
        records: Iterable[FinanceComplianceScoringRecord] | None = None,
        rules: Iterable[FinanceComplianceScoringRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceComplianceScoringRecord] = {}
        self._rules: dict[str, FinanceComplianceScoringRule] = {}
    ...
    def add_rule(self, rule: FinanceComplianceScoringRule) -> FinanceComplianceScoringRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceComplianceScoringRecord, *, evaluate: bool = True) -> FinanceComplianceScoringRecord:
    ...
    def get(self, record_id: object) -> FinanceComplianceScoringRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceComplianceScoringRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceComplianceScoringRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceComplianceScoringEvidence) -> FinanceComplianceScoringRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceComplianceScoringRecord]:
        updated: list[FinanceComplianceScoringRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceComplianceScoringRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceComplianceScoringRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceComplianceScoringRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceComplianceScoringRule, ...]:
    return (
        FinanceComplianceScoringRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceComplianceScoringRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceComplianceScoringRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceComplianceScoringRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_compliance_scoring")
    return FinanceComplianceScoringRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_compliance_scoring",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_compliance_scoring",),
    )


def demo_register() -> FinanceComplianceScoringRegister:
    register = FinanceComplianceScoringRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_compliance_scoring record", "campus.office", "1250", priority="urgent"))
    return register
