...
class FinanceEvidenceAuditRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceEvidenceAuditRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceEvidenceAuditEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceEvidenceAuditEvidence":
        return cls(
...
@dataclass(frozen=True)
class FinanceEvidenceAuditEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceEvidenceAuditEvent":
        return cls(
...
@dataclass(frozen=True)
class FinanceEvidenceAuditRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[FinanceEvidenceAuditEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceEvidenceAuditEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceEvidenceAuditRule]) -> "FinanceEvidenceAuditRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceEvidenceAuditRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceEvidenceAuditRecord":
        status_key = clean_status(status)
        event = FinanceEvidenceAuditEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceEvidenceAuditRecord":
    ...
    def add_evidence(self, evidence: FinanceEvidenceAuditEvidence) -> "FinanceEvidenceAuditRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceEvidenceAuditRecord":
        return cls(
            ...
            evidence=tuple(FinanceEvidenceAuditEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceEvidenceAuditEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceEvidenceAuditRegister:
    domain = "finance_evidence_audit"

    def __init__(
        self,
        records: Iterable[FinanceEvidenceAuditRecord] | None = None,
        rules: Iterable[FinanceEvidenceAuditRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceEvidenceAuditRecord] = {}
        self._rules: dict[str, FinanceEvidenceAuditRule] = {}
    ...
    def add_rule(self, rule: FinanceEvidenceAuditRule) -> FinanceEvidenceAuditRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceEvidenceAuditRecord, *, evaluate: bool = True) -> FinanceEvidenceAuditRecord:
    ...
    def get(self, record_id: object) -> FinanceEvidenceAuditRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceEvidenceAuditRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceEvidenceAuditRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceEvidenceAuditEvidence) -> FinanceEvidenceAuditRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceEvidenceAuditRecord]:
        updated: list[FinanceEvidenceAuditRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceEvidenceAuditRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceEvidenceAuditRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceEvidenceAuditRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceEvidenceAuditRule, ...]:
    return (
        FinanceEvidenceAuditRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceEvidenceAuditRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceEvidenceAuditRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceEvidenceAuditRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_evidence_audit")
    return FinanceEvidenceAuditRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_evidence_audit",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_evidence_audit",),
    )


def demo_register() -> FinanceEvidenceAuditRegister:
    register = FinanceEvidenceAuditRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_evidence_audit record", "campus.office", "1250", priority="urgent"))
    return register
