...
class StudentClearanceControlRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentClearanceControlRule":
        return cls(
...
@dataclass(frozen=True)
class StudentClearanceControlEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentClearanceControlEvidence":
        return cls(
...
@dataclass(frozen=True)
class StudentClearanceControlEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentClearanceControlEvent":
        return cls(
...
@dataclass(frozen=True)
class StudentClearanceControlRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[StudentClearanceControlEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentClearanceControlEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentClearanceControlRule]) -> "StudentClearanceControlRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentClearanceControlRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentClearanceControlRecord":
        status_key = clean_status(status)
        event = StudentClearanceControlEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentClearanceControlRecord":
    ...
    def add_evidence(self, evidence: StudentClearanceControlEvidence) -> "StudentClearanceControlRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentClearanceControlRecord":
        return cls(
            ...
            evidence=tuple(StudentClearanceControlEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentClearanceControlEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentClearanceControlRegister:
    domain = "student_clearance_control"

    def __init__(
        self,
        records: Iterable[StudentClearanceControlRecord] | None = None,
        rules: Iterable[StudentClearanceControlRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentClearanceControlRecord] = {}
        self._rules: dict[str, StudentClearanceControlRule] = {}
    ...
    def add_rule(self, rule: StudentClearanceControlRule) -> StudentClearanceControlRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentClearanceControlRecord, *, evaluate: bool = True) -> StudentClearanceControlRecord:
    ...
    def get(self, record_id: object) -> StudentClearanceControlRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentClearanceControlRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentClearanceControlRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentClearanceControlEvidence) -> StudentClearanceControlRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentClearanceControlRecord]:
        updated: list[StudentClearanceControlRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentClearanceControlRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentClearanceControlRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentClearanceControlRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentClearanceControlRule, ...]:
    return (
        StudentClearanceControlRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentClearanceControlRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentClearanceControlRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentClearanceControlRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_clearance_control")
    return StudentClearanceControlRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_clearance_control",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_clearance_control",),
    )


def demo_register() -> StudentClearanceControlRegister:
    register = StudentClearanceControlRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_clearance_control record", "campus.office", "1250", priority="urgent"))
    return register
