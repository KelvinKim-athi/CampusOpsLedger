...
class StudentCaseTriageRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentCaseTriageRule":
        return cls(
...
@dataclass(frozen=True)
class StudentCaseTriageEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentCaseTriageEvidence":
        return cls(
...
class StudentCaseTriageEvent:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentCaseTriageEvent":
        return cls(
...
class StudentCaseTriageRecord:
    ...
    evidence: tuple[StudentCaseTriageEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentCaseTriageEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentCaseTriageRule]) -> "StudentCaseTriageRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentCaseTriageRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentCaseTriageRecord":
        status_key = clean_status(status)
        event = StudentCaseTriageEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentCaseTriageRecord":
    ...
    def add_evidence(self, evidence: StudentCaseTriageEvidence) -> "StudentCaseTriageRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentCaseTriageRecord":
        return cls(
            ...
            evidence=tuple(StudentCaseTriageEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentCaseTriageEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentCaseTriageRegister:
    domain = "student_case_triage"

    def __init__(
        self,
        records: Iterable[StudentCaseTriageRecord] | None = None,
        rules: Iterable[StudentCaseTriageRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentCaseTriageRecord] = {}
        self._rules: dict[str, StudentCaseTriageRule] = {}
    ...
    def add_rule(self, rule: StudentCaseTriageRule) -> StudentCaseTriageRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentCaseTriageRecord, *, evaluate: bool = True) -> StudentCaseTriageRecord:
    ...
    def get(self, record_id: object) -> StudentCaseTriageRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentCaseTriageRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentCaseTriageRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentCaseTriageEvidence) -> StudentCaseTriageRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentCaseTriageRecord]:
        updated: list[StudentCaseTriageRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentCaseTriageRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentCaseTriageRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentCaseTriageRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentCaseTriageRule, ...]:
    return (
        StudentCaseTriageRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentCaseTriageRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentCaseTriageRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentCaseTriageRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_case_triage")
    return StudentCaseTriageRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_case_triage",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_case_triage",),
    )


def demo_register() -> StudentCaseTriageRegister:
    register = StudentCaseTriageRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_case_triage record", "campus.office", "1250", priority="urgent"))
    return register
