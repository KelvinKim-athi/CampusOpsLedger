...
class FinanceEligibilityReviewRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceEligibilityReviewRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceEligibilityReviewEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceEligibilityReviewEvidence":
        return cls(
...
@dataclass(frozen=True)
class FinanceEligibilityReviewEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceEligibilityReviewEvent":
        return cls(
...
@dataclass(frozen=True)
class FinanceEligibilityReviewRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[FinanceEligibilityReviewEvidence, ...] = field(default_factory=tuple)
    events: tuple[FinanceEligibilityReviewEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[FinanceEligibilityReviewRule]) -> "FinanceEligibilityReviewRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return FinanceEligibilityReviewRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceEligibilityReviewRecord":
        status_key = clean_status(status)
        event = FinanceEligibilityReviewEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "FinanceEligibilityReviewRecord":
    ...
    def add_evidence(self, evidence: FinanceEligibilityReviewEvidence) -> "FinanceEligibilityReviewRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceEligibilityReviewRecord":
        return cls(
            ...
            evidence=tuple(FinanceEligibilityReviewEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(FinanceEligibilityReviewEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FinanceEligibilityReviewRegister:
    domain = "finance_eligibility_review"

    def __init__(
        self,
        records: Iterable[FinanceEligibilityReviewRecord] | None = None,
        rules: Iterable[FinanceEligibilityReviewRule] | None = None,
    ) -> None:
        self._records: dict[str, FinanceEligibilityReviewRecord] = {}
        self._rules: dict[str, FinanceEligibilityReviewRule] = {}
    ...
    def add_rule(self, rule: FinanceEligibilityReviewRule) -> FinanceEligibilityReviewRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: FinanceEligibilityReviewRecord, *, evaluate: bool = True) -> FinanceEligibilityReviewRecord:
    ...
    def get(self, record_id: object) -> FinanceEligibilityReviewRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FinanceEligibilityReviewRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FinanceEligibilityReviewRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: FinanceEligibilityReviewEvidence) -> FinanceEligibilityReviewRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[FinanceEligibilityReviewRecord]:
        updated: list[FinanceEligibilityReviewRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceEligibilityReviewRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceEligibilityReviewRule.from_dict(row) for row in payload.get("rules", ())]
        records = [FinanceEligibilityReviewRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[FinanceEligibilityReviewRule, ...]:
    return (
        FinanceEligibilityReviewRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        FinanceEligibilityReviewRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        FinanceEligibilityReviewRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> FinanceEligibilityReviewRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "finance_eligibility_review")
    return FinanceEligibilityReviewRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="finance_eligibility_review",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("finance_eligibility_review",),
    )


def demo_register() -> FinanceEligibilityReviewRegister:
    register = FinanceEligibilityReviewRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo finance_eligibility_review record", "campus.office", "1250", priority="urgent"))
    return register
