...
class StudentEligibilityReviewRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentEligibilityReviewRule":
        return cls(
...
@dataclass(frozen=True)
class StudentEligibilityReviewEvidence:
    evidence_id: str
    submitted_by: str
    description: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentEligibilityReviewEvidence":
        return cls(
...
@dataclass(frozen=True)
class StudentEligibilityReviewEvent:
    event_id: str
    actor: str
    event_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentEligibilityReviewEvent":
        return cls(
...
@dataclass(frozen=True)
class StudentEligibilityReviewRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    evidence: tuple[StudentEligibilityReviewEvidence, ...] = field(default_factory=tuple)
    events: tuple[StudentEligibilityReviewEvent, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def evaluate_rules(self, rules: Iterable[StudentEligibilityReviewRule]) -> "StudentEligibilityReviewRecord":
        matched = tuple(sorted(rule.rule_id for rule in rules if rule.matches(self.payload)))
        return StudentEligibilityReviewRecord(
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentEligibilityReviewRecord":
        status_key = clean_status(status)
        event = StudentEligibilityReviewEvent(
    ...
    def assign_owner(self, owner: object, *, actor: object) -> "StudentEligibilityReviewRecord":
    ...
    def add_evidence(self, evidence: StudentEligibilityReviewEvidence) -> "StudentEligibilityReviewRecord":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentEligibilityReviewRecord":
        return cls(
            ...
            evidence=tuple(StudentEligibilityReviewEvidence.from_dict(row) for row in payload.get("evidence", ())),
            events=tuple(StudentEligibilityReviewEvent.from_dict(row) for row in payload.get("events", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentEligibilityReviewRegister:
    domain = "student_eligibility_review"

    def __init__(
        self,
        records: Iterable[StudentEligibilityReviewRecord] | None = None,
        rules: Iterable[StudentEligibilityReviewRule] | None = None,
    ) -> None:
        self._records: dict[str, StudentEligibilityReviewRecord] = {}
        self._rules: dict[str, StudentEligibilityReviewRule] = {}
    ...
    def add_rule(self, rule: StudentEligibilityReviewRule) -> StudentEligibilityReviewRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_record(self, record: StudentEligibilityReviewRecord, *, evaluate: bool = True) -> StudentEligibilityReviewRecord:
    ...
    def get(self, record_id: object) -> StudentEligibilityReviewRecord:
    ...
    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentEligibilityReviewRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentEligibilityReviewRecord:
        record = self.get(record_id).assign_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def add_evidence(self, record_id: object, evidence: StudentEligibilityReviewEvidence) -> StudentEligibilityReviewRecord:
        record = self.get(record_id).add_evidence(evidence)
        self._records[record.record_id] = record
        return record

    def reevaluate(self) -> list[StudentEligibilityReviewRecord]:
        updated: list[StudentEligibilityReviewRecord] = []
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentEligibilityReviewRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentEligibilityReviewRule.from_dict(row) for row in payload.get("rules", ())]
        records = [StudentEligibilityReviewRecord.from_dict(row) for row in payload.get("records", ())]
...
def default_rules() -> tuple[StudentEligibilityReviewRule, ...]:
    return (
        StudentEligibilityReviewRule("amount-threshold", "Amount threshold", "amount", "gte", "1000", score=25, tags=("finance",)),
        StudentEligibilityReviewRule("source-required", "Source required", "source", "exists", "", score=5, required=True),
        StudentEligibilityReviewRule("priority-marker", "Urgent marker", "priority", "equals", "urgent", score=10, tags=("risk",)),
    )
...
def build_record(
    ...
    value: object = "0.00",
    **payload: object,
) -> StudentEligibilityReviewRecord:
    data = dict(payload)
    data.setdefault("amount", str(value))
    data.setdefault("source", "student_eligibility_review")
    return StudentEligibilityReviewRecord(
        record_id=record_id,
        subject_id=subject_id,
        title=title,
        owner=owner,
        category="student_eligibility_review",
        severity=NORMAL,
        value=value,
        payload=data,
        tags=("student_eligibility_review",),
    )


def demo_register() -> StudentEligibilityReviewRegister:
    register = StudentEligibilityReviewRegister(rules=default_rules())
    register.add_record(build_record("DEMO-1", "S001", "Demo student_eligibility_review record", "campus.office", "1250", priority="urgent"))
    return register
