...
class BudgetVarianceRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "BudgetVarianceRule":
        return cls(
...
class BudgetVarianceAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "BudgetVarianceAction":
        return cls(
...
class BudgetVarianceCase:
    ...
    actions: tuple[BudgetVarianceAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "BudgetVarianceCase":
        status_key = clean_status(status)
        action = BudgetVarianceAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "BudgetVarianceCase":
    ...
    def with_rule_matches(self, rules: Iterable[BudgetVarianceRule]) -> "BudgetVarianceCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "BudgetVarianceCase":
        return cls(
            ...
            actions=tuple(BudgetVarianceAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class BudgetVarianceEngine:
    def __init__(
        self,
        cases: Iterable[BudgetVarianceCase] | None = None,
        rules: Iterable[BudgetVarianceRule] | None = None,
    ) -> None:
        self._cases: dict[str, BudgetVarianceCase] = {}
        self._rules: dict[str, BudgetVarianceRule] = {}
    ...
    def add_rule(self, rule: BudgetVarianceRule) -> BudgetVarianceRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: BudgetVarianceCase, *, evaluate: bool = True) -> BudgetVarianceCase:
    ...
    def get_case(self, case_id: object) -> BudgetVarianceCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> BudgetVarianceCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> BudgetVarianceCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[BudgetVarianceCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[BudgetVarianceCase] = []
    ...
    def cases_by_status(self, status: object) -> list[BudgetVarianceCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[BudgetVarianceCase]:
    ...
    def open_cases(self) -> list[BudgetVarianceCase]:
    ...
    def closed_cases(self) -> list[BudgetVarianceCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "BudgetVarianceEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [BudgetVarianceRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [BudgetVarianceCase.from_dict(row) for row in payload.get("cases", ())]
