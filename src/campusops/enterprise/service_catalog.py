...
class ServiceCatalogRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ServiceCatalogRule":
        return cls(
...
class ServiceCatalogAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ServiceCatalogAction":
        return cls(
...
class ServiceCatalogCase:
    ...
    actions: tuple[ServiceCatalogAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ServiceCatalogCase":
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ServiceCatalogCase":
    ...
    def with_rule_matches(self, rules: Iterable[ServiceCatalogRule]) -> "ServiceCatalogCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ServiceCatalogCase":
        return cls(
            ...
            actions=tuple(ServiceCatalogAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class ServiceCatalogEngine:
    def __init__(
        self,
        cases: Iterable[ServiceCatalogCase] | None = None,
        rules: Iterable[ServiceCatalogRule] | None = None,
    ) -> None:
        self._cases: dict[str, ServiceCatalogCase] = {}
        self._rules: dict[str, ServiceCatalogRule] = {}
    ...
    def add_rule(self, rule: ServiceCatalogRule) -> ServiceCatalogRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: ServiceCatalogCase, *, evaluate: bool = True) -> ServiceCatalogCase:
    ...
    def get_case(self, case_id: object) -> ServiceCatalogCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> ServiceCatalogCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> ServiceCatalogCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[ServiceCatalogCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[ServiceCatalogCase] = []
    ...
    def cases_by_status(self, status: object) -> list[ServiceCatalogCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[ServiceCatalogCase]:
    ...
    def open_cases(self) -> list[ServiceCatalogCase]:
    ...
    def closed_cases(self) -> list[ServiceCatalogCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ServiceCatalogEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [ServiceCatalogRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [ServiceCatalogCase.from_dict(row) for row in payload.get("cases", ())]
