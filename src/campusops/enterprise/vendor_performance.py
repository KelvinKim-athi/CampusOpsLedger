...
class VendorPerformanceRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VendorPerformanceRule":
        return cls(
...
class VendorPerformanceAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VendorPerformanceAction":
        return cls(
...
class VendorPerformanceCase:
    ...
    actions: tuple[VendorPerformanceAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "VendorPerformanceCase":
        status_key = clean_status(status)
        action = VendorPerformanceAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "VendorPerformanceCase":
    ...
    def with_rule_matches(self, rules: Iterable[VendorPerformanceRule]) -> "VendorPerformanceCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VendorPerformanceCase":
        return cls(
            ...
            actions=tuple(VendorPerformanceAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class VendorPerformanceEngine:
    def __init__(
        self,
        cases: Iterable[VendorPerformanceCase] | None = None,
        rules: Iterable[VendorPerformanceRule] | None = None,
    ) -> None:
        self._cases: dict[str, VendorPerformanceCase] = {}
        self._rules: dict[str, VendorPerformanceRule] = {}
    ...
    def add_rule(self, rule: VendorPerformanceRule) -> VendorPerformanceRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: VendorPerformanceCase, *, evaluate: bool = True) -> VendorPerformanceCase:
    ...
    def get_case(self, case_id: object) -> VendorPerformanceCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> VendorPerformanceCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> VendorPerformanceCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[VendorPerformanceCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[VendorPerformanceCase] = []
    ...
    def cases_by_status(self, status: object) -> list[VendorPerformanceCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[VendorPerformanceCase]:
    ...
    def open_cases(self) -> list[VendorPerformanceCase]:
    ...
    def closed_cases(self) -> list[VendorPerformanceCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "VendorPerformanceEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [VendorPerformanceRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [VendorPerformanceCase.from_dict(row) for row in payload.get("cases", ())]
