...
class DocumentRetentionRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DocumentRetentionRule":
        return cls(
...
class DocumentRetentionAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DocumentRetentionAction":
        return cls(
...
class DocumentRetentionCase:
    ...
    actions: tuple[DocumentRetentionAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "DocumentRetentionCase":
    ...
    def with_owner(self, owner: object, *, actor: object) -> "DocumentRetentionCase":
    ...
    def with_rule_matches(self, rules: Iterable[DocumentRetentionRule]) -> "DocumentRetentionCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DocumentRetentionCase":
        return cls(
            ...
            actions=tuple(DocumentRetentionAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class DocumentRetentionEngine:
    def __init__(
        self,
        cases: Iterable[DocumentRetentionCase] | None = None,
        rules: Iterable[DocumentRetentionRule] | None = None,
    ) -> None:
        self._cases: dict[str, DocumentRetentionCase] = {}
        self._rules: dict[str, DocumentRetentionRule] = {}
    ...
    def add_rule(self, rule: DocumentRetentionRule) -> DocumentRetentionRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: DocumentRetentionCase, *, evaluate: bool = True) -> DocumentRetentionCase:
    ...
    def get_case(self, case_id: object) -> DocumentRetentionCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> DocumentRetentionCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> DocumentRetentionCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[DocumentRetentionCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[DocumentRetentionCase] = []
    ...
    def cases_by_status(self, status: object) -> list[DocumentRetentionCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[DocumentRetentionCase]:
    ...
    def open_cases(self) -> list[DocumentRetentionCase]:
    ...
    def closed_cases(self) -> list[DocumentRetentionCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "DocumentRetentionEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [DocumentRetentionRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [DocumentRetentionCase.from_dict(row) for row in payload.get("cases", ())]
