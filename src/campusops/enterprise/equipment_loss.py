...
class EquipmentLossRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EquipmentLossRule":
        return cls(
...
class EquipmentLossAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EquipmentLossAction":
        return cls(
...
class EquipmentLossCase:
    ...
    actions: tuple[EquipmentLossAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "EquipmentLossCase":
        status_key = clean_status(status)
        action = EquipmentLossAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "EquipmentLossCase":
    ...
    def with_rule_matches(self, rules: Iterable[EquipmentLossRule]) -> "EquipmentLossCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EquipmentLossCase":
        return cls(
            ...
            actions=tuple(EquipmentLossAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class EquipmentLossEngine:
    def __init__(
        self,
        cases: Iterable[EquipmentLossCase] | None = None,
        rules: Iterable[EquipmentLossRule] | None = None,
    ) -> None:
        self._cases: dict[str, EquipmentLossCase] = {}
        self._rules: dict[str, EquipmentLossRule] = {}
    ...
    def add_rule(self, rule: EquipmentLossRule) -> EquipmentLossRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: EquipmentLossCase, *, evaluate: bool = True) -> EquipmentLossCase:
    ...
    def get_case(self, case_id: object) -> EquipmentLossCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> EquipmentLossCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> EquipmentLossCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[EquipmentLossCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[EquipmentLossCase] = []
    ...
    def cases_by_status(self, status: object) -> list[EquipmentLossCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[EquipmentLossCase]:
    ...
    def open_cases(self) -> list[EquipmentLossCase]:
    ...
    def closed_cases(self) -> list[EquipmentLossCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "EquipmentLossEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [EquipmentLossRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [EquipmentLossCase.from_dict(row) for row in payload.get("cases", ())]
