...
class ParkingEnforcementRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ParkingEnforcementRule":
        return cls(
...
@dataclass(frozen=True)
class ParkingEnforcementAction:
    action_id: str
    actor: str
    action_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ParkingEnforcementAction":
        return cls(
...
class ParkingEnforcementCase:
    ...
    actions: tuple[ParkingEnforcementAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ParkingEnforcementCase":
        status_key = clean_status(status)
        action = ParkingEnforcementAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ParkingEnforcementCase":
    ...
    def with_rule_matches(self, rules: Iterable[ParkingEnforcementRule]) -> "ParkingEnforcementCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ParkingEnforcementCase":
        return cls(
            ...
            actions=tuple(ParkingEnforcementAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class ParkingEnforcementEngine:
    def __init__(
        self,
        cases: Iterable[ParkingEnforcementCase] | None = None,
        rules: Iterable[ParkingEnforcementRule] | None = None,
    ) -> None:
        self._cases: dict[str, ParkingEnforcementCase] = {}
        self._rules: dict[str, ParkingEnforcementRule] = {}
    ...
    def add_rule(self, rule: ParkingEnforcementRule) -> ParkingEnforcementRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: ParkingEnforcementCase, *, evaluate: bool = True) -> ParkingEnforcementCase:
    ...
    def get_case(self, case_id: object) -> ParkingEnforcementCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> ParkingEnforcementCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> ParkingEnforcementCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[ParkingEnforcementCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[ParkingEnforcementCase] = []
    ...
    def cases_by_status(self, status: object) -> list[ParkingEnforcementCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[ParkingEnforcementCase]:
    ...
    def open_cases(self) -> list[ParkingEnforcementCase]:
    ...
    def closed_cases(self) -> list[ParkingEnforcementCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ParkingEnforcementEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [ParkingEnforcementRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [ParkingEnforcementCase.from_dict(row) for row in payload.get("cases", ())]
