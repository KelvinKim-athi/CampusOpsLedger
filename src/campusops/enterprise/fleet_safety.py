...
class FleetSafetyRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FleetSafetyRule":
        return cls(
...
class FleetSafetyAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FleetSafetyAction":
        return cls(
...
class FleetSafetyCase:
    ...
    actions: tuple[FleetSafetyAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FleetSafetyCase":
    ...
    def with_owner(self, owner: object, *, actor: object) -> "FleetSafetyCase":
    ...
    def with_rule_matches(self, rules: Iterable[FleetSafetyRule]) -> "FleetSafetyCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FleetSafetyCase":
        return cls(
            ...
            actions=tuple(FleetSafetyAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class FleetSafetyEngine:
    def __init__(
        self,
        cases: Iterable[FleetSafetyCase] | None = None,
        rules: Iterable[FleetSafetyRule] | None = None,
    ) -> None:
        self._cases: dict[str, FleetSafetyCase] = {}
        self._rules: dict[str, FleetSafetyRule] = {}
    ...
    def add_rule(self, rule: FleetSafetyRule) -> FleetSafetyRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: FleetSafetyCase, *, evaluate: bool = True) -> FleetSafetyCase:
    ...
    def get_case(self, case_id: object) -> FleetSafetyCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> FleetSafetyCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> FleetSafetyCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[FleetSafetyCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[FleetSafetyCase] = []
    ...
    def cases_by_status(self, status: object) -> list[FleetSafetyCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[FleetSafetyCase]:
    ...
    def open_cases(self) -> list[FleetSafetyCase]:
    ...
    def closed_cases(self) -> list[FleetSafetyCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FleetSafetyEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FleetSafetyRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [FleetSafetyCase.from_dict(row) for row in payload.get("cases", ())]
