...
class DataQualityRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DataQualityRule":
        return cls(
...
class DataQualityAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DataQualityAction":
        return cls(
...
class DataQualityCase:
    ...
    actions: tuple[DataQualityAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "DataQualityCase":
    ...
    def with_owner(self, owner: object, *, actor: object) -> "DataQualityCase":
    ...
    def with_rule_matches(self, rules: Iterable[DataQualityRule]) -> "DataQualityCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DataQualityCase":
        return cls(
            ...
            actions=tuple(DataQualityAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class DataQualityEngine:
    def __init__(
        self,
        cases: Iterable[DataQualityCase] | None = None,
        rules: Iterable[DataQualityRule] | None = None,
    ) -> None:
        self._cases: dict[str, DataQualityCase] = {}
    ...
    def add_rule(self, rule: DataQualityRule) -> DataQualityRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: DataQualityCase, *, evaluate: bool = True) -> DataQualityCase:
    ...
    def get_case(self, case_id: object) -> DataQualityCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> DataQualityCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> DataQualityCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[DataQualityCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[DataQualityCase] = []
    ...
    def cases_by_status(self, status: object) -> list[DataQualityCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[DataQualityCase]:
    ...
    def open_cases(self) -> list[DataQualityCase]:
    ...
    def closed_cases(self) -> list[DataQualityCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "DataQualityEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [DataQualityRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [DataQualityCase.from_dict(row) for row in payload.get("cases", ())]
