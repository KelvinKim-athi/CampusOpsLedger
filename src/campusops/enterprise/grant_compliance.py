...
class GrantComplianceRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GrantComplianceRule":
        return cls(
...
class GrantComplianceAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GrantComplianceAction":
        return cls(
...
class GrantComplianceCase:
    ...
    actions: tuple[GrantComplianceAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "GrantComplianceCase":
        status_key = clean_status(status)
        action = GrantComplianceAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "GrantComplianceCase":
    ...
    def with_rule_matches(self, rules: Iterable[GrantComplianceRule]) -> "GrantComplianceCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GrantComplianceCase":
        return cls(
            ...
            actions=tuple(GrantComplianceAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class GrantComplianceEngine:
    def __init__(
        self,
        cases: Iterable[GrantComplianceCase] | None = None,
        rules: Iterable[GrantComplianceRule] | None = None,
    ) -> None:
        self._cases: dict[str, GrantComplianceCase] = {}
        self._rules: dict[str, GrantComplianceRule] = {}
    ...
    def add_rule(self, rule: GrantComplianceRule) -> GrantComplianceRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: GrantComplianceCase, *, evaluate: bool = True) -> GrantComplianceCase:
    ...
    def get_case(self, case_id: object) -> GrantComplianceCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> GrantComplianceCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> GrantComplianceCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[GrantComplianceCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[GrantComplianceCase] = []
    ...
    def cases_by_status(self, status: object) -> list[GrantComplianceCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[GrantComplianceCase]:
    ...
    def open_cases(self) -> list[GrantComplianceCase]:
    ...
    def closed_cases(self) -> list[GrantComplianceCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "GrantComplianceEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [GrantComplianceRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [GrantComplianceCase.from_dict(row) for row in payload.get("cases", ())]
