...
class LibraryFinesRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "LibraryFinesRule":
        return cls(
...
class LibraryFinesAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "LibraryFinesAction":
        return cls(
...
class LibraryFinesCase:
    ...
    actions: tuple[LibraryFinesAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "LibraryFinesCase":
        status_key = clean_status(status)
        action = LibraryFinesAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "LibraryFinesCase":
    ...
    def with_rule_matches(self, rules: Iterable[LibraryFinesRule]) -> "LibraryFinesCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "LibraryFinesCase":
        return cls(
            ...
            actions=tuple(LibraryFinesAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class LibraryFinesEngine:
    def __init__(
        self,
        cases: Iterable[LibraryFinesCase] | None = None,
        rules: Iterable[LibraryFinesRule] | None = None,
    ) -> None:
        self._cases: dict[str, LibraryFinesCase] = {}
        self._rules: dict[str, LibraryFinesRule] = {}
    ...
    def add_rule(self, rule: LibraryFinesRule) -> LibraryFinesRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: LibraryFinesCase, *, evaluate: bool = True) -> LibraryFinesCase:
    ...
    def get_case(self, case_id: object) -> LibraryFinesCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> LibraryFinesCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> LibraryFinesCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[LibraryFinesCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[LibraryFinesCase] = []
    ...
    def cases_by_status(self, status: object) -> list[LibraryFinesCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[LibraryFinesCase]:
    ...
    def open_cases(self) -> list[LibraryFinesCase]:
    ...
    def closed_cases(self) -> list[LibraryFinesCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "LibraryFinesEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [LibraryFinesRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [LibraryFinesCase.from_dict(row) for row in payload.get("cases", ())]
