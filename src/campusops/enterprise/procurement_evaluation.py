...
class ProcurementEvaluationRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ProcurementEvaluationRule":
        return cls(
...
@dataclass(frozen=True)
class ProcurementEvaluationAction:
    action_id: str
    actor: str
    action_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ProcurementEvaluationAction":
        return cls(
...
@dataclass(frozen=True)
class ProcurementEvaluationCase:
    case_id: str
    subject_id: str
    summary: str
    owner: str
    ...
    actions: tuple[ProcurementEvaluationAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ProcurementEvaluationCase":
        status_key = clean_status(status)
        action = ProcurementEvaluationAction(
        ...
        return ProcurementEvaluationCase(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ProcurementEvaluationCase":
    ...
    def with_rule_matches(self, rules: Iterable[ProcurementEvaluationRule]) -> "ProcurementEvaluationCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ProcurementEvaluationCase":
        return cls(
            ...
            actions=tuple(ProcurementEvaluationAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class ProcurementEvaluationEngine:
    def __init__(
        self,
        cases: Iterable[ProcurementEvaluationCase] | None = None,
        rules: Iterable[ProcurementEvaluationRule] | None = None,
    ) -> None:
        self._cases: dict[str, ProcurementEvaluationCase] = {}
        self._rules: dict[str, ProcurementEvaluationRule] = {}
    ...
    def add_rule(self, rule: ProcurementEvaluationRule) -> ProcurementEvaluationRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: ProcurementEvaluationCase, *, evaluate: bool = True) -> ProcurementEvaluationCase:
    ...
    def get_case(self, case_id: object) -> ProcurementEvaluationCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> ProcurementEvaluationCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> ProcurementEvaluationCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[ProcurementEvaluationCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[ProcurementEvaluationCase] = []
    ...
    def cases_by_status(self, status: object) -> list[ProcurementEvaluationCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[ProcurementEvaluationCase]:
    ...
    def open_cases(self) -> list[ProcurementEvaluationCase]:
    ...
    def closed_cases(self) -> list[ProcurementEvaluationCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ProcurementEvaluationEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [ProcurementEvaluationRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [ProcurementEvaluationCase.from_dict(row) for row in payload.get("cases", ())]
