...
class PayrollExceptionRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "PayrollExceptionRule":
        return cls(
...
class PayrollExceptionAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "PayrollExceptionAction":
        return cls(
...
class PayrollExceptionCase:
    ...
    actions: tuple[PayrollExceptionAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "PayrollExceptionCase":
        status_key = clean_status(status)
        action = PayrollExceptionAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "PayrollExceptionCase":
    ...
    def with_rule_matches(self, rules: Iterable[PayrollExceptionRule]) -> "PayrollExceptionCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "PayrollExceptionCase":
        return cls(
            ...
            actions=tuple(PayrollExceptionAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class PayrollExceptionEngine:
    def __init__(
        self,
        cases: Iterable[PayrollExceptionCase] | None = None,
        rules: Iterable[PayrollExceptionRule] | None = None,
    ) -> None:
        self._cases: dict[str, PayrollExceptionCase] = {}
        self._rules: dict[str, PayrollExceptionRule] = {}
    ...
    def add_rule(self, rule: PayrollExceptionRule) -> PayrollExceptionRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: PayrollExceptionCase, *, evaluate: bool = True) -> PayrollExceptionCase:
    ...
    def get_case(self, case_id: object) -> PayrollExceptionCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> PayrollExceptionCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> PayrollExceptionCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[PayrollExceptionCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[PayrollExceptionCase] = []
    ...
    def cases_by_status(self, status: object) -> list[PayrollExceptionCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[PayrollExceptionCase]:
    ...
    def open_cases(self) -> list[PayrollExceptionCase]:
    ...
    def closed_cases(self) -> list[PayrollExceptionCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "PayrollExceptionEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [PayrollExceptionRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [PayrollExceptionCase.from_dict(row) for row in payload.get("cases", ())]
