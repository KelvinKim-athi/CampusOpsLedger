...
class InternshipReviewRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "InternshipReviewRule":
        return cls(
...
class InternshipReviewAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "InternshipReviewAction":
        return cls(
...
class InternshipReviewCase:
    ...
    actions: tuple[InternshipReviewAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "InternshipReviewCase":
        status_key = clean_status(status)
        action = InternshipReviewAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "InternshipReviewCase":
    ...
    def with_rule_matches(self, rules: Iterable[InternshipReviewRule]) -> "InternshipReviewCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "InternshipReviewCase":
        return cls(
            ...
            actions=tuple(InternshipReviewAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class InternshipReviewEngine:
    def __init__(
        self,
        cases: Iterable[InternshipReviewCase] | None = None,
        rules: Iterable[InternshipReviewRule] | None = None,
    ) -> None:
        self._cases: dict[str, InternshipReviewCase] = {}
        self._rules: dict[str, InternshipReviewRule] = {}
    ...
    def add_rule(self, rule: InternshipReviewRule) -> InternshipReviewRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: InternshipReviewCase, *, evaluate: bool = True) -> InternshipReviewCase:
    ...
    def get_case(self, case_id: object) -> InternshipReviewCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> InternshipReviewCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> InternshipReviewCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[InternshipReviewCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[InternshipReviewCase] = []
    ...
    def cases_by_status(self, status: object) -> list[InternshipReviewCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[InternshipReviewCase]:
    ...
    def open_cases(self) -> list[InternshipReviewCase]:
    ...
    def closed_cases(self) -> list[InternshipReviewCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "internship_review",
            "case_type": "placement_case",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "InternshipReviewEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [InternshipReviewRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [InternshipReviewCase.from_dict(row) for row in payload.get("cases", ())]
