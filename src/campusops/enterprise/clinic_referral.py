...
class ClinicReferralRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ClinicReferralRule":
        return cls(
...
class ClinicReferralAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ClinicReferralAction":
        return cls(
...
class ClinicReferralCase:
    ...
    actions: tuple[ClinicReferralAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ClinicReferralCase":
        status_key = clean_status(status)
        action = ClinicReferralAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ClinicReferralCase":
    ...
    def with_rule_matches(self, rules: Iterable[ClinicReferralRule]) -> "ClinicReferralCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ClinicReferralCase":
        return cls(
            ...
            actions=tuple(ClinicReferralAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class ClinicReferralEngine:
    def __init__(
        self,
        cases: Iterable[ClinicReferralCase] | None = None,
        rules: Iterable[ClinicReferralRule] | None = None,
    ) -> None:
        self._cases: dict[str, ClinicReferralCase] = {}
        self._rules: dict[str, ClinicReferralRule] = {}
    ...
    def add_rule(self, rule: ClinicReferralRule) -> ClinicReferralRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: ClinicReferralCase, *, evaluate: bool = True) -> ClinicReferralCase:
    ...
    def get_case(self, case_id: object) -> ClinicReferralCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> ClinicReferralCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> ClinicReferralCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[ClinicReferralCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[ClinicReferralCase] = []
    ...
    def cases_by_status(self, status: object) -> list[ClinicReferralCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[ClinicReferralCase]:
    ...
    def open_cases(self) -> list[ClinicReferralCase]:
    ...
    def closed_cases(self) -> list[ClinicReferralCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ClinicReferralEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [ClinicReferralRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [ClinicReferralCase.from_dict(row) for row in payload.get("cases", ())]
