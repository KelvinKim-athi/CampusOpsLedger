...
class CafeteriaCreditRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CafeteriaCreditRule":
        return cls(
...
class CafeteriaCreditAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CafeteriaCreditAction":
        return cls(
...
class CafeteriaCreditCase:
    ...
    actions: tuple[CafeteriaCreditAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "CafeteriaCreditCase":
        status_key = clean_status(status)
        action = CafeteriaCreditAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "CafeteriaCreditCase":
    ...
    def with_rule_matches(self, rules: Iterable[CafeteriaCreditRule]) -> "CafeteriaCreditCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CafeteriaCreditCase":
        return cls(
            ...
            actions=tuple(CafeteriaCreditAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class CafeteriaCreditEngine:
    def __init__(
        self,
        cases: Iterable[CafeteriaCreditCase] | None = None,
        rules: Iterable[CafeteriaCreditRule] | None = None,
    ) -> None:
        self._cases: dict[str, CafeteriaCreditCase] = {}
        self._rules: dict[str, CafeteriaCreditRule] = {}
    ...
    def add_rule(self, rule: CafeteriaCreditRule) -> CafeteriaCreditRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: CafeteriaCreditCase, *, evaluate: bool = True) -> CafeteriaCreditCase:
    ...
    def get_case(self, case_id: object) -> CafeteriaCreditCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> CafeteriaCreditCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> CafeteriaCreditCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[CafeteriaCreditCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[CafeteriaCreditCase] = []
    ...
    def cases_by_status(self, status: object) -> list[CafeteriaCreditCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[CafeteriaCreditCase]:
    ...
    def open_cases(self) -> list[CafeteriaCreditCase]:
    ...
    def closed_cases(self) -> list[CafeteriaCreditCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "cafeteria_credit",
            "case_type": "credit_case",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "CafeteriaCreditEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [CafeteriaCreditRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [CafeteriaCreditCase.from_dict(row) for row in payload.get("cases", ())]
