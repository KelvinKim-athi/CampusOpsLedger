...
class AppealsBoardRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AppealsBoardRule":
        return cls(
...
class AppealsBoardAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AppealsBoardAction":
        return cls(
...
class AppealsBoardCase:
    ...
    actions: tuple[AppealsBoardAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "AppealsBoardCase":
        status_key = clean_status(status)
        action = AppealsBoardAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "AppealsBoardCase":
    ...
    def with_rule_matches(self, rules: Iterable[AppealsBoardRule]) -> "AppealsBoardCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AppealsBoardCase":
        return cls(
            ...
            actions=tuple(AppealsBoardAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class AppealsBoardEngine:
    def __init__(
        self,
        cases: Iterable[AppealsBoardCase] | None = None,
        rules: Iterable[AppealsBoardRule] | None = None,
    ) -> None:
        self._cases: dict[str, AppealsBoardCase] = {}
        self._rules: dict[str, AppealsBoardRule] = {}
    ...
    def add_rule(self, rule: AppealsBoardRule) -> AppealsBoardRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: AppealsBoardCase, *, evaluate: bool = True) -> AppealsBoardCase:
    ...
    def get_case(self, case_id: object) -> AppealsBoardCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> AppealsBoardCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> AppealsBoardCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[AppealsBoardCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[AppealsBoardCase] = []
    ...
    def cases_by_status(self, status: object) -> list[AppealsBoardCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[AppealsBoardCase]:
    ...
    def open_cases(self) -> list[AppealsBoardCase]:
    ...
    def closed_cases(self) -> list[AppealsBoardCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "AppealsBoardEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [AppealsBoardRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [AppealsBoardCase.from_dict(row) for row in payload.get("cases", ())]
