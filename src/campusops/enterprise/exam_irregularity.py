...
class ExamIrregularityRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ExamIrregularityRule":
        return cls(
...
class ExamIrregularityAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ExamIrregularityAction":
        return cls(
...
class ExamIrregularityCase:
    ...
    actions: tuple[ExamIrregularityAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ExamIrregularityCase":
        status_key = clean_status(status)
        action = ExamIrregularityAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ExamIrregularityCase":
    ...
    def with_rule_matches(self, rules: Iterable[ExamIrregularityRule]) -> "ExamIrregularityCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ExamIrregularityCase":
        return cls(
            ...
            actions=tuple(ExamIrregularityAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class ExamIrregularityEngine:
    def __init__(
        self,
        cases: Iterable[ExamIrregularityCase] | None = None,
        rules: Iterable[ExamIrregularityRule] | None = None,
    ) -> None:
        self._cases: dict[str, ExamIrregularityCase] = {}
        self._rules: dict[str, ExamIrregularityRule] = {}
    ...
    def add_rule(self, rule: ExamIrregularityRule) -> ExamIrregularityRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: ExamIrregularityCase, *, evaluate: bool = True) -> ExamIrregularityCase:
    ...
    def get_case(self, case_id: object) -> ExamIrregularityCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> ExamIrregularityCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> ExamIrregularityCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[ExamIrregularityCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[ExamIrregularityCase] = []
    ...
    def cases_by_status(self, status: object) -> list[ExamIrregularityCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[ExamIrregularityCase]:
    ...
    def open_cases(self) -> list[ExamIrregularityCase]:
    ...
    def closed_cases(self) -> list[ExamIrregularityCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ExamIrregularityEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [ExamIrregularityRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [ExamIrregularityCase.from_dict(row) for row in payload.get("cases", ())]
