...
class VisitorScreeningRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VisitorScreeningRule":
        return cls(
...
class VisitorScreeningAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VisitorScreeningAction":
        return cls(
...
class VisitorScreeningCase:
    ...
    actions: tuple[VisitorScreeningAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "VisitorScreeningCase":
        status_key = clean_status(status)
        action = VisitorScreeningAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "VisitorScreeningCase":
    ...
    def with_rule_matches(self, rules: Iterable[VisitorScreeningRule]) -> "VisitorScreeningCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VisitorScreeningCase":
        return cls(
            ...
            actions=tuple(VisitorScreeningAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class VisitorScreeningEngine:
    def __init__(
        self,
        cases: Iterable[VisitorScreeningCase] | None = None,
        rules: Iterable[VisitorScreeningRule] | None = None,
    ) -> None:
        self._cases: dict[str, VisitorScreeningCase] = {}
        self._rules: dict[str, VisitorScreeningRule] = {}
    ...
    def add_rule(self, rule: VisitorScreeningRule) -> VisitorScreeningRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: VisitorScreeningCase, *, evaluate: bool = True) -> VisitorScreeningCase:
    ...
    def get_case(self, case_id: object) -> VisitorScreeningCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> VisitorScreeningCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> VisitorScreeningCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[VisitorScreeningCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[VisitorScreeningCase] = []
    ...
    def cases_by_status(self, status: object) -> list[VisitorScreeningCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[VisitorScreeningCase]:
    ...
    def open_cases(self) -> list[VisitorScreeningCase]:
    ...
    def closed_cases(self) -> list[VisitorScreeningCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "visitor_screening",
            "case_type": "visit_case",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "VisitorScreeningEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [VisitorScreeningRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [VisitorScreeningCase.from_dict(row) for row in payload.get("cases", ())]
