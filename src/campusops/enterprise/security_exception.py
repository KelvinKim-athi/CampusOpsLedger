...
class SecurityExceptionRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "SecurityExceptionRule":
        return cls(
...
class SecurityExceptionAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "SecurityExceptionAction":
        return cls(
...
class SecurityExceptionCase:
    ...
    actions: tuple[SecurityExceptionAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "SecurityExceptionCase":
        status_key = clean_status(status)
        action = SecurityExceptionAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "SecurityExceptionCase":
    ...
    def with_rule_matches(self, rules: Iterable[SecurityExceptionRule]) -> "SecurityExceptionCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "SecurityExceptionCase":
        return cls(
            ...
            actions=tuple(SecurityExceptionAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class SecurityExceptionEngine:
    def __init__(
        self,
        cases: Iterable[SecurityExceptionCase] | None = None,
        rules: Iterable[SecurityExceptionRule] | None = None,
    ) -> None:
        self._cases: dict[str, SecurityExceptionCase] = {}
        self._rules: dict[str, SecurityExceptionRule] = {}
    ...
    def add_rule(self, rule: SecurityExceptionRule) -> SecurityExceptionRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: SecurityExceptionCase, *, evaluate: bool = True) -> SecurityExceptionCase:
    ...
    def get_case(self, case_id: object) -> SecurityExceptionCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> SecurityExceptionCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> SecurityExceptionCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[SecurityExceptionCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[SecurityExceptionCase] = []
    ...
    def cases_by_status(self, status: object) -> list[SecurityExceptionCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[SecurityExceptionCase]:
    ...
    def open_cases(self) -> list[SecurityExceptionCase]:
    ...
    def closed_cases(self) -> list[SecurityExceptionCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "security_exception",
            "case_type": "exception_case",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "SecurityExceptionEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [SecurityExceptionRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [SecurityExceptionCase.from_dict(row) for row in payload.get("cases", ())]
