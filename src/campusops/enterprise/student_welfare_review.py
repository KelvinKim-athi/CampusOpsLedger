...
class StudentWelfareReviewRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentWelfareReviewRule":
        return cls(
...
@dataclass(frozen=True)
class StudentWelfareReviewAction:
    action_id: str
    actor: str
    action_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentWelfareReviewAction":
        return cls(
...
@dataclass(frozen=True)
class StudentWelfareReviewCase:
    case_id: str
    subject_id: str
    summary: str
    owner: str
    ...
    actions: tuple[StudentWelfareReviewAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentWelfareReviewCase":
        status_key = clean_status(status)
        action = StudentWelfareReviewAction(
        ...
        return StudentWelfareReviewCase(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "StudentWelfareReviewCase":
    ...
    def with_rule_matches(self, rules: Iterable[StudentWelfareReviewRule]) -> "StudentWelfareReviewCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentWelfareReviewCase":
        return cls(
            ...
            actions=tuple(StudentWelfareReviewAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class StudentWelfareReviewEngine:
    def __init__(
        self,
        cases: Iterable[StudentWelfareReviewCase] | None = None,
        rules: Iterable[StudentWelfareReviewRule] | None = None,
    ) -> None:
        self._cases: dict[str, StudentWelfareReviewCase] = {}
        self._rules: dict[str, StudentWelfareReviewRule] = {}
    ...
    def add_rule(self, rule: StudentWelfareReviewRule) -> StudentWelfareReviewRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: StudentWelfareReviewCase, *, evaluate: bool = True) -> StudentWelfareReviewCase:
    ...
    def get_case(self, case_id: object) -> StudentWelfareReviewCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> StudentWelfareReviewCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> StudentWelfareReviewCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[StudentWelfareReviewCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[StudentWelfareReviewCase] = []
    ...
    def cases_by_status(self, status: object) -> list[StudentWelfareReviewCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[StudentWelfareReviewCase]:
    ...
    def open_cases(self) -> list[StudentWelfareReviewCase]:
    ...
    def closed_cases(self) -> list[StudentWelfareReviewCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "student_welfare_review",
            "case_type": "welfare_case",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentWelfareReviewEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [StudentWelfareReviewRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [StudentWelfareReviewCase.from_dict(row) for row in payload.get("cases", ())]
