...
class FacilityRepairRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FacilityRepairRule":
        return cls(
...
class FacilityRepairAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FacilityRepairAction":
        return cls(
...
class FacilityRepairCase:
    ...
    actions: tuple[FacilityRepairAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FacilityRepairCase":
        status_key = clean_status(status)
        action = FacilityRepairAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "FacilityRepairCase":
    ...
    def with_rule_matches(self, rules: Iterable[FacilityRepairRule]) -> "FacilityRepairCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FacilityRepairCase":
        return cls(
            ...
            actions=tuple(FacilityRepairAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class FacilityRepairEngine:
    def __init__(
        self,
        cases: Iterable[FacilityRepairCase] | None = None,
        rules: Iterable[FacilityRepairRule] | None = None,
    ) -> None:
        self._cases: dict[str, FacilityRepairCase] = {}
        self._rules: dict[str, FacilityRepairRule] = {}
    ...
    def add_rule(self, rule: FacilityRepairRule) -> FacilityRepairRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: FacilityRepairCase, *, evaluate: bool = True) -> FacilityRepairCase:
    ...
    def get_case(self, case_id: object) -> FacilityRepairCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> FacilityRepairCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> FacilityRepairCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[FacilityRepairCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[FacilityRepairCase] = []
    ...
    def cases_by_status(self, status: object) -> list[FacilityRepairCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[FacilityRepairCase]:
    ...
    def open_cases(self) -> list[FacilityRepairCase]:
    ...
    def closed_cases(self) -> list[FacilityRepairCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FacilityRepairEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FacilityRepairRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [FacilityRepairCase.from_dict(row) for row in payload.get("cases", ())]
