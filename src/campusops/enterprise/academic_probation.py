...
class AcademicProbationRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AcademicProbationRule":
        return cls(
...
class AcademicProbationAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AcademicProbationAction":
        return cls(
...
class AcademicProbationCase:
    ...
    actions: tuple[AcademicProbationAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "AcademicProbationCase":
        status_key = clean_status(status)
        action = AcademicProbationAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "AcademicProbationCase":
    ...
    def with_rule_matches(self, rules: Iterable[AcademicProbationRule]) -> "AcademicProbationCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AcademicProbationCase":
        return cls(
            ...
            actions=tuple(AcademicProbationAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class AcademicProbationEngine:
    def __init__(
        self,
        cases: Iterable[AcademicProbationCase] | None = None,
        rules: Iterable[AcademicProbationRule] | None = None,
    ) -> None:
        self._cases: dict[str, AcademicProbationCase] = {}
        self._rules: dict[str, AcademicProbationRule] = {}
    ...
    def add_rule(self, rule: AcademicProbationRule) -> AcademicProbationRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: AcademicProbationCase, *, evaluate: bool = True) -> AcademicProbationCase:
    ...
    def get_case(self, case_id: object) -> AcademicProbationCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> AcademicProbationCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> AcademicProbationCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[AcademicProbationCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[AcademicProbationCase] = []
    ...
    def cases_by_status(self, status: object) -> list[AcademicProbationCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[AcademicProbationCase]:
    ...
    def open_cases(self) -> list[AcademicProbationCase]:
    ...
    def closed_cases(self) -> list[AcademicProbationCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "academic_probation",
            "case_type": "probation_case",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "AcademicProbationEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [AcademicProbationRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [AcademicProbationCase.from_dict(row) for row in payload.get("cases", ())]
