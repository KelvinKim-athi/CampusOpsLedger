...
class AttendanceInterventionRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AttendanceInterventionRule":
        return cls(
...
@dataclass(frozen=True)
class AttendanceInterventionAction:
    action_id: str
    actor: str
    action_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AttendanceInterventionAction":
        return cls(
...
@dataclass(frozen=True)
class AttendanceInterventionCase:
    case_id: str
    subject_id: str
    summary: str
    owner: str
    ...
    actions: tuple[AttendanceInterventionAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "AttendanceInterventionCase":
        status_key = clean_status(status)
        action = AttendanceInterventionAction(
        ...
        return AttendanceInterventionCase(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "AttendanceInterventionCase":
    ...
    def with_rule_matches(self, rules: Iterable[AttendanceInterventionRule]) -> "AttendanceInterventionCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AttendanceInterventionCase":
        return cls(
            ...
            actions=tuple(AttendanceInterventionAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class AttendanceInterventionEngine:
    def __init__(
        self,
        cases: Iterable[AttendanceInterventionCase] | None = None,
        rules: Iterable[AttendanceInterventionRule] | None = None,
    ) -> None:
        self._cases: dict[str, AttendanceInterventionCase] = {}
        self._rules: dict[str, AttendanceInterventionRule] = {}
    ...
    def add_rule(self, rule: AttendanceInterventionRule) -> AttendanceInterventionRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: AttendanceInterventionCase, *, evaluate: bool = True) -> AttendanceInterventionCase:
    ...
    def get_case(self, case_id: object) -> AttendanceInterventionCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> AttendanceInterventionCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> AttendanceInterventionCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[AttendanceInterventionCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[AttendanceInterventionCase] = []
    ...
    def cases_by_status(self, status: object) -> list[AttendanceInterventionCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[AttendanceInterventionCase]:
    ...
    def open_cases(self) -> list[AttendanceInterventionCase]:
    ...
    def closed_cases(self) -> list[AttendanceInterventionCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "attendance_intervention",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "AttendanceInterventionEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [AttendanceInterventionRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [AttendanceInterventionCase.from_dict(row) for row in payload.get("cases", ())]
