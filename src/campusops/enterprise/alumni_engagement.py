...
class AlumniEngagementRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AlumniEngagementRule":
        return cls(
...
class AlumniEngagementAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AlumniEngagementAction":
        return cls(
...
class AlumniEngagementCase:
    ...
    actions: tuple[AlumniEngagementAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "AlumniEngagementCase":
        status_key = clean_status(status)
        action = AlumniEngagementAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "AlumniEngagementCase":
    ...
    def with_rule_matches(self, rules: Iterable[AlumniEngagementRule]) -> "AlumniEngagementCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AlumniEngagementCase":
        return cls(
            ...
            actions=tuple(AlumniEngagementAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class AlumniEngagementEngine:
    def __init__(
        self,
        cases: Iterable[AlumniEngagementCase] | None = None,
        rules: Iterable[AlumniEngagementRule] | None = None,
    ) -> None:
        self._cases: dict[str, AlumniEngagementCase] = {}
        self._rules: dict[str, AlumniEngagementRule] = {}
    ...
    def add_rule(self, rule: AlumniEngagementRule) -> AlumniEngagementRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: AlumniEngagementCase, *, evaluate: bool = True) -> AlumniEngagementCase:
    ...
    def get_case(self, case_id: object) -> AlumniEngagementCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> AlumniEngagementCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> AlumniEngagementCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[AlumniEngagementCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[AlumniEngagementCase] = []
    ...
    def cases_by_status(self, status: object) -> list[AlumniEngagementCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[AlumniEngagementCase]:
    ...
    def open_cases(self) -> list[AlumniEngagementCase]:
    ...
    def closed_cases(self) -> list[AlumniEngagementCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "alumni_engagement",
            "case_type": "engagement_case",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "AlumniEngagementEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [AlumniEngagementRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [AlumniEngagementCase.from_dict(row) for row in payload.get("cases", ())]
