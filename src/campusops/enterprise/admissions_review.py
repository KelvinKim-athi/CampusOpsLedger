...
class AdmissionsReviewRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AdmissionsReviewRule":
        return cls(
...
class AdmissionsReviewAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AdmissionsReviewAction":
        return cls(
...
class AdmissionsReviewCase:
    ...
    actions: tuple[AdmissionsReviewAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "AdmissionsReviewCase":
        status_key = clean_status(status)
        action = AdmissionsReviewAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "AdmissionsReviewCase":
    ...
    def with_rule_matches(self, rules: Iterable[AdmissionsReviewRule]) -> "AdmissionsReviewCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AdmissionsReviewCase":
        return cls(
            ...
            actions=tuple(AdmissionsReviewAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class AdmissionsReviewEngine:
    def __init__(
        self,
        cases: Iterable[AdmissionsReviewCase] | None = None,
        rules: Iterable[AdmissionsReviewRule] | None = None,
    ) -> None:
        self._cases: dict[str, AdmissionsReviewCase] = {}
        self._rules: dict[str, AdmissionsReviewRule] = {}
    ...
    def add_rule(self, rule: AdmissionsReviewRule) -> AdmissionsReviewRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: AdmissionsReviewCase, *, evaluate: bool = True) -> AdmissionsReviewCase:
    ...
    def get_case(self, case_id: object) -> AdmissionsReviewCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> AdmissionsReviewCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> AdmissionsReviewCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[AdmissionsReviewCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[AdmissionsReviewCase] = []
    ...
    def cases_by_status(self, status: object) -> list[AdmissionsReviewCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[AdmissionsReviewCase]:
    ...
    def open_cases(self) -> list[AdmissionsReviewCase]:
    ...
    def closed_cases(self) -> list[AdmissionsReviewCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "AdmissionsReviewEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [AdmissionsReviewRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [AdmissionsReviewCase.from_dict(row) for row in payload.get("cases", ())]
