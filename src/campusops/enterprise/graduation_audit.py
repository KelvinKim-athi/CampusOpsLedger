...
class GraduationAuditRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GraduationAuditRule":
        return cls(
...
class GraduationAuditAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GraduationAuditAction":
        return cls(
...
class GraduationAuditCase:
    ...
    actions: tuple[GraduationAuditAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "GraduationAuditCase":
        status_key = clean_status(status)
        action = GraduationAuditAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "GraduationAuditCase":
    ...
    def with_rule_matches(self, rules: Iterable[GraduationAuditRule]) -> "GraduationAuditCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GraduationAuditCase":
        return cls(
            ...
            actions=tuple(GraduationAuditAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class GraduationAuditEngine:
    def __init__(
        self,
        cases: Iterable[GraduationAuditCase] | None = None,
        rules: Iterable[GraduationAuditRule] | None = None,
    ) -> None:
        self._cases: dict[str, GraduationAuditCase] = {}
        self._rules: dict[str, GraduationAuditRule] = {}
    ...
    def add_rule(self, rule: GraduationAuditRule) -> GraduationAuditRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: GraduationAuditCase, *, evaluate: bool = True) -> GraduationAuditCase:
    ...
    def get_case(self, case_id: object) -> GraduationAuditCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> GraduationAuditCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> GraduationAuditCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[GraduationAuditCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[GraduationAuditCase] = []
    ...
    def cases_by_status(self, status: object) -> list[GraduationAuditCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[GraduationAuditCase]:
    ...
    def open_cases(self) -> list[GraduationAuditCase]:
    ...
    def closed_cases(self) -> list[GraduationAuditCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "GraduationAuditEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [GraduationAuditRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [GraduationAuditCase.from_dict(row) for row in payload.get("cases", ())]
