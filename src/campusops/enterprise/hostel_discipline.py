...
class HostelDisciplineRule:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "HostelDisciplineRule":
        return cls(
...
class HostelDisciplineAction:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "HostelDisciplineAction":
        return cls(
...
class HostelDisciplineCase:
    ...
    actions: tuple[HostelDisciplineAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "HostelDisciplineCase":
        status_key = clean_status(status)
        action = HostelDisciplineAction(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "HostelDisciplineCase":
    ...
    def with_rule_matches(self, rules: Iterable[HostelDisciplineRule]) -> "HostelDisciplineCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "HostelDisciplineCase":
        return cls(
            ...
            actions=tuple(HostelDisciplineAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )


class HostelDisciplineEngine:
    def __init__(
        self,
        cases: Iterable[HostelDisciplineCase] | None = None,
        rules: Iterable[HostelDisciplineRule] | None = None,
    ) -> None:
        self._cases: dict[str, HostelDisciplineCase] = {}
        self._rules: dict[str, HostelDisciplineRule] = {}
    ...
    def add_rule(self, rule: HostelDisciplineRule) -> HostelDisciplineRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: HostelDisciplineCase, *, evaluate: bool = True) -> HostelDisciplineCase:
    ...
    def get_case(self, case_id: object) -> HostelDisciplineCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> HostelDisciplineCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> HostelDisciplineCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[HostelDisciplineCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[HostelDisciplineCase] = []
    ...
    def cases_by_status(self, status: object) -> list[HostelDisciplineCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[HostelDisciplineCase]:
    ...
    def open_cases(self) -> list[HostelDisciplineCase]:
    ...
    def closed_cases(self) -> list[HostelDisciplineCase]:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "domain": "hostel_discipline",
            "case_type": "discipline_case",
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "HostelDisciplineEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [HostelDisciplineRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [HostelDisciplineCase.from_dict(row) for row in payload.get("cases", ())]
