...
class FinanceReconciliationRule:
    rule_id: str
    title: str
    field: str
    operator: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceReconciliationRule":
        return cls(
...
@dataclass(frozen=True)
class FinanceReconciliationAction:
    action_id: str
    actor: str
    action_type: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceReconciliationAction":
        return cls(
...
@dataclass(frozen=True)
class FinanceReconciliationCase:
    case_id: str
    subject_id: str
    summary: str
    owner: str
    ...
    actions: tuple[FinanceReconciliationAction, ...] = field(default_factory=tuple)
    tags: tuple[str, ...] = field(default_factory=tuple)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FinanceReconciliationCase":
        ...
        return FinanceReconciliationCase(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "FinanceReconciliationCase":
    ...
    def with_rule_matches(self, rules: Iterable[FinanceReconciliationRule]) -> "FinanceReconciliationCase":
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FinanceReconciliationCase":
        return cls(
            ...
            actions=tuple(FinanceReconciliationAction.from_dict(row) for row in payload.get("actions", ())),
            tags=tuple(payload.get("tags") or ()),
        )
...
class FinanceReconciliationEngine:
    def __init__(
        self,
        cases: Iterable[FinanceReconciliationCase] | None = None,
        rules: Iterable[FinanceReconciliationRule] | None = None,
    ) -> None:
        self._cases: dict[str, FinanceReconciliationCase] = {}
        self._rules: dict[str, FinanceReconciliationRule] = {}
    ...
    def add_rule(self, rule: FinanceReconciliationRule) -> FinanceReconciliationRule:
        if rule.rule_id in self._rules:
            raise ValueError(f"rule already exists: {rule.rule_id}")
        self._rules[rule.rule_id] = rule
        return rule

    def add_case(self, case: FinanceReconciliationCase, *, evaluate: bool = True) -> FinanceReconciliationCase:
    ...
    def get_case(self, case_id: object) -> FinanceReconciliationCase:
    ...
    def update_status(self, case_id: object, status: object, *, actor: object, message: object = "") -> FinanceReconciliationCase:
        case = self.get_case(case_id).with_status(status, actor=actor, message=message)
        self._cases[case.case_id] = case
        return case

    def assign_owner(self, case_id: object, owner: object, *, actor: object) -> FinanceReconciliationCase:
        case = self.get_case(case_id).with_owner(owner, actor=actor)
        self._cases[case.case_id] = case
        return case

    def reevaluate(self, case_id: object | None = None) -> list[FinanceReconciliationCase]:
        selected = [self.get_case(case_id)] if case_id is not None else list(self._cases.values())
        updated: list[FinanceReconciliationCase] = []
    ...
    def cases_by_status(self, status: object) -> list[FinanceReconciliationCase]:
    ...
    def cases_by_owner(self, owner: object) -> list[FinanceReconciliationCase]:
    ...
    def open_cases(self) -> list[FinanceReconciliationCase]:
    ...
    def closed_cases(self) -> list[FinanceReconciliationCase]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FinanceReconciliationEngine":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        rules = [FinanceReconciliationRule.from_dict(row) for row in payload.get("rules", ())]
        cases = [FinanceReconciliationCase.from_dict(row) for row in payload.get("cases", ())]
