...
class VisitorManagementNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VisitorManagementNote":
        return cls(
...
@dataclass(frozen=True)
class VisitorManagementHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VisitorManagementHistoryEntry":
        return cls(
...
class VisitorManagementRecord:
    ...
    notes: tuple[VisitorManagementNote, ...] = field(default_factory=tuple)
    history: tuple[VisitorManagementHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "VisitorManagementRecord":
        status_key = clean_status(status)
        entry = VisitorManagementHistoryEntry(
        ...
        return VisitorManagementRecord(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "VisitorManagementRecord":
    ...
    def with_note(self, note: VisitorManagementNote) -> "VisitorManagementRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "VisitorManagementRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VisitorManagementRecord":
        return cls(
            ...
            notes=tuple(VisitorManagementNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(VisitorManagementHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class VisitorManagementRegister:
    def __init__(self, records: Iterable[VisitorManagementRecord] | None = None) -> None:
        self._records: dict[str, VisitorManagementRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: VisitorManagementRecord) -> VisitorManagementRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> VisitorManagementRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> VisitorManagementRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> VisitorManagementRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: VisitorManagementNote) -> VisitorManagementRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> VisitorManagementRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[VisitorManagementRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[VisitorManagementRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[VisitorManagementRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[VisitorManagementRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "VisitorManagementRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(VisitorManagementRecord.from_dict(row) for row in payload.get("records", ()))
