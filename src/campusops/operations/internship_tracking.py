...
class InternshipTrackingNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "InternshipTrackingNote":
        return cls(
...
@dataclass(frozen=True)
class InternshipTrackingHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "InternshipTrackingHistoryEntry":
        return cls(
...
@dataclass(frozen=True)
class InternshipTrackingRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    notes: tuple[InternshipTrackingNote, ...] = field(default_factory=tuple)
    history: tuple[InternshipTrackingHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "InternshipTrackingRecord":
        status_key = clean_status(status)
        entry = InternshipTrackingHistoryEntry(
        ...
        return InternshipTrackingRecord(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "InternshipTrackingRecord":
    ...
    def with_note(self, note: InternshipTrackingNote) -> "InternshipTrackingRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "InternshipTrackingRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "InternshipTrackingRecord":
        return cls(
            ...
            notes=tuple(InternshipTrackingNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(InternshipTrackingHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class InternshipTrackingRegister:
    def __init__(self, records: Iterable[InternshipTrackingRecord] | None = None) -> None:
        self._records: dict[str, InternshipTrackingRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: InternshipTrackingRecord) -> InternshipTrackingRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> InternshipTrackingRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> InternshipTrackingRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> InternshipTrackingRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: InternshipTrackingNote) -> InternshipTrackingRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> InternshipTrackingRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[InternshipTrackingRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[InternshipTrackingRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[InternshipTrackingRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[InternshipTrackingRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "InternshipTrackingRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(InternshipTrackingRecord.from_dict(row) for row in payload.get("records", ()))
