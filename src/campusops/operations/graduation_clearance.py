...
class GraduationClearanceNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GraduationClearanceNote":
        return cls(
...
@dataclass(frozen=True)
class GraduationClearanceHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GraduationClearanceHistoryEntry":
        return cls(
...
@dataclass(frozen=True)
class GraduationClearanceRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    notes: tuple[GraduationClearanceNote, ...] = field(default_factory=tuple)
    history: tuple[GraduationClearanceHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "GraduationClearanceRecord":
        status_key = clean_status(status)
        entry = GraduationClearanceHistoryEntry(
        ...
        return GraduationClearanceRecord(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "GraduationClearanceRecord":
    ...
    def with_note(self, note: GraduationClearanceNote) -> "GraduationClearanceRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "GraduationClearanceRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "GraduationClearanceRecord":
        return cls(
            ...
            notes=tuple(GraduationClearanceNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(GraduationClearanceHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class GraduationClearanceRegister:
    def __init__(self, records: Iterable[GraduationClearanceRecord] | None = None) -> None:
        self._records: dict[str, GraduationClearanceRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: GraduationClearanceRecord) -> GraduationClearanceRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> GraduationClearanceRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> GraduationClearanceRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> GraduationClearanceRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: GraduationClearanceNote) -> GraduationClearanceRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> GraduationClearanceRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[GraduationClearanceRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[GraduationClearanceRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[GraduationClearanceRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[GraduationClearanceRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "GraduationClearanceRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(GraduationClearanceRecord.from_dict(row) for row in payload.get("records", ()))
