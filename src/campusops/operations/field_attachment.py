...
class FieldAttachmentNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FieldAttachmentNote":
        return cls(
...
@dataclass(frozen=True)
class FieldAttachmentHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FieldAttachmentHistoryEntry":
        return cls(
...
class FieldAttachmentRecord:
    ...
    notes: tuple[FieldAttachmentNote, ...] = field(default_factory=tuple)
    history: tuple[FieldAttachmentHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FieldAttachmentRecord":
        status_key = clean_status(status)
        entry = FieldAttachmentHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "FieldAttachmentRecord":
    ...
    def with_note(self, note: FieldAttachmentNote) -> "FieldAttachmentRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "FieldAttachmentRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FieldAttachmentRecord":
        return cls(
            ...
            notes=tuple(FieldAttachmentNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(FieldAttachmentHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class FieldAttachmentRegister:
    def __init__(self, records: Iterable[FieldAttachmentRecord] | None = None) -> None:
        self._records: dict[str, FieldAttachmentRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: FieldAttachmentRecord) -> FieldAttachmentRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> FieldAttachmentRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FieldAttachmentRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FieldAttachmentRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: FieldAttachmentNote) -> FieldAttachmentRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> FieldAttachmentRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[FieldAttachmentRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[FieldAttachmentRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[FieldAttachmentRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[FieldAttachmentRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FieldAttachmentRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(FieldAttachmentRecord.from_dict(row) for row in payload.get("records", ()))
