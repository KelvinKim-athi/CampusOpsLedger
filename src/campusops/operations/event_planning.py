...
class EventPlanningNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EventPlanningNote":
        return cls(
...
@dataclass(frozen=True)
class EventPlanningHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EventPlanningHistoryEntry":
        return cls(
...
class EventPlanningRecord:
    ...
    notes: tuple[EventPlanningNote, ...] = field(default_factory=tuple)
    history: tuple[EventPlanningHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "EventPlanningRecord":
        status_key = clean_status(status)
        entry = EventPlanningHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "EventPlanningRecord":
    ...
    def with_note(self, note: EventPlanningNote) -> "EventPlanningRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "EventPlanningRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EventPlanningRecord":
        return cls(
            ...
            notes=tuple(EventPlanningNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(EventPlanningHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class EventPlanningRegister:
    def __init__(self, records: Iterable[EventPlanningRecord] | None = None) -> None:
        self._records: dict[str, EventPlanningRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: EventPlanningRecord) -> EventPlanningRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> EventPlanningRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> EventPlanningRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> EventPlanningRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: EventPlanningNote) -> EventPlanningRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> EventPlanningRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[EventPlanningRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[EventPlanningRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[EventPlanningRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[EventPlanningRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "EventPlanningRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(EventPlanningRecord.from_dict(row) for row in payload.get("records", ()))
