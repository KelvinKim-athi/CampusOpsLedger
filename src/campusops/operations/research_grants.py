...
class ResearchGrantNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ResearchGrantNote":
        return cls(
...
@dataclass(frozen=True)
class ResearchGrantHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ResearchGrantHistoryEntry":
        return cls(
...
class ResearchGrantRecord:
    ...
    notes: tuple[ResearchGrantNote, ...] = field(default_factory=tuple)
    history: tuple[ResearchGrantHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ResearchGrantRecord":
        status_key = clean_status(status)
        entry = ResearchGrantHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ResearchGrantRecord":
    ...
    def with_note(self, note: ResearchGrantNote) -> "ResearchGrantRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "ResearchGrantRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ResearchGrantRecord":
        return cls(
            ...
            notes=tuple(ResearchGrantNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(ResearchGrantHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class ResearchGrantRegister:
    def __init__(self, records: Iterable[ResearchGrantRecord] | None = None) -> None:
        self._records: dict[str, ResearchGrantRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: ResearchGrantRecord) -> ResearchGrantRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> ResearchGrantRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> ResearchGrantRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> ResearchGrantRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: ResearchGrantNote) -> ResearchGrantRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> ResearchGrantRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[ResearchGrantRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[ResearchGrantRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[ResearchGrantRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[ResearchGrantRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ResearchGrantRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(ResearchGrantRecord.from_dict(row) for row in payload.get("records", ()))
