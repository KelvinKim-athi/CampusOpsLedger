...
class OperationNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "OperationNote":
        return cls(
...
class OperationHistoryEntry:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "OperationHistoryEntry":
        return cls(
...
class OperationRecord:
    ...
    notes: tuple[OperationNote, ...] = field(default_factory=tuple)
    history: tuple[OperationHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "OperationRecord":
    ...
    def with_owner(self, owner: object, *, actor: object) -> "OperationRecord":
    ...
    def with_note(self, note: OperationNote) -> "OperationRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "OperationRecord":
        metadata = deepcopy(self.metadata)
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "OperationRecord":
        return cls(
            ...
            notes=tuple(OperationNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(OperationHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )
...
class OperationRegister:
    ...
    def __init__(self, records: Iterable[OperationRecord] | None = None) -> None:
    ...
    def get(self, record_id: object) -> OperationRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> OperationRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> OperationRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: OperationNote) -> OperationRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> OperationRecord:
    ...
    def validate_integrity(self) -> dict[str, Any]:
        return {"record_count": len(self._records), "valid": True}
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "OperationRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(cls.record_class.from_dict(row) for row in payload.get("records", ()))
