...
class HostelAllocationNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "HostelAllocationNote":
        return cls(
...
@dataclass(frozen=True)
class HostelAllocationHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "HostelAllocationHistoryEntry":
        return cls(
...
class HostelAllocationRecord:
    ...
    notes: tuple[HostelAllocationNote, ...] = field(default_factory=tuple)
    history: tuple[HostelAllocationHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "HostelAllocationRecord":
        status_key = clean_status(status)
        entry = HostelAllocationHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "HostelAllocationRecord":
    ...
    def with_note(self, note: HostelAllocationNote) -> "HostelAllocationRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "HostelAllocationRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "HostelAllocationRecord":
        return cls(
            ...
            notes=tuple(HostelAllocationNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(HostelAllocationHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class HostelAllocationRegister:
    def __init__(self, records: Iterable[HostelAllocationRecord] | None = None) -> None:
        self._records: dict[str, HostelAllocationRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: HostelAllocationRecord) -> HostelAllocationRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> HostelAllocationRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> HostelAllocationRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> HostelAllocationRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: HostelAllocationNote) -> HostelAllocationRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> HostelAllocationRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[HostelAllocationRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[HostelAllocationRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[HostelAllocationRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[HostelAllocationRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "HostelAllocationRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(HostelAllocationRecord.from_dict(row) for row in payload.get("records", ()))
