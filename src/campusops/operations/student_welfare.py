...
class StudentWelfareNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentWelfareNote":
        return cls(
...
@dataclass(frozen=True)
class StudentWelfareHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentWelfareHistoryEntry":
        return cls(
...
class StudentWelfareRecord:
    ...
    notes: tuple[StudentWelfareNote, ...] = field(default_factory=tuple)
    history: tuple[StudentWelfareHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StudentWelfareRecord":
        status_key = clean_status(status)
        entry = StudentWelfareHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "StudentWelfareRecord":
    ...
    def with_note(self, note: StudentWelfareNote) -> "StudentWelfareRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "StudentWelfareRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StudentWelfareRecord":
        return cls(
            ...
            notes=tuple(StudentWelfareNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(StudentWelfareHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class StudentWelfareRegister:
    def __init__(self, records: Iterable[StudentWelfareRecord] | None = None) -> None:
        self._records: dict[str, StudentWelfareRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: StudentWelfareRecord) -> StudentWelfareRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> StudentWelfareRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StudentWelfareRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StudentWelfareRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: StudentWelfareNote) -> StudentWelfareRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> StudentWelfareRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[StudentWelfareRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[StudentWelfareRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[StudentWelfareRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[StudentWelfareRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StudentWelfareRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(StudentWelfareRecord.from_dict(row) for row in payload.get("records", ()))
