...
class FacilityInspectionNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FacilityInspectionNote":
        return cls(
...
@dataclass(frozen=True)
class FacilityInspectionHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FacilityInspectionHistoryEntry":
        return cls(
...
@dataclass(frozen=True)
class FacilityInspectionRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    notes: tuple[FacilityInspectionNote, ...] = field(default_factory=tuple)
    history: tuple[FacilityInspectionHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "FacilityInspectionRecord":
        status_key = clean_status(status)
        entry = FacilityInspectionHistoryEntry(
        ...
        return FacilityInspectionRecord(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "FacilityInspectionRecord":
    ...
    def with_note(self, note: FacilityInspectionNote) -> "FacilityInspectionRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "FacilityInspectionRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "FacilityInspectionRecord":
        return cls(
            ...
            notes=tuple(FacilityInspectionNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(FacilityInspectionHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class FacilityInspectionRegister:
    def __init__(self, records: Iterable[FacilityInspectionRecord] | None = None) -> None:
        self._records: dict[str, FacilityInspectionRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: FacilityInspectionRecord) -> FacilityInspectionRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> FacilityInspectionRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> FacilityInspectionRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> FacilityInspectionRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: FacilityInspectionNote) -> FacilityInspectionRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> FacilityInspectionRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[FacilityInspectionRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[FacilityInspectionRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[FacilityInspectionRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[FacilityInspectionRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "FacilityInspectionRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(FacilityInspectionRecord.from_dict(row) for row in payload.get("records", ()))
