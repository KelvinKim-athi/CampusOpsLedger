...
class BudgetPlanningNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "BudgetPlanningNote":
        return cls(
...
@dataclass(frozen=True)
class BudgetPlanningHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "BudgetPlanningHistoryEntry":
        return cls(
...
class BudgetPlanningRecord:
    ...
    notes: tuple[BudgetPlanningNote, ...] = field(default_factory=tuple)
    history: tuple[BudgetPlanningHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "BudgetPlanningRecord":
        status_key = clean_status(status)
        entry = BudgetPlanningHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "BudgetPlanningRecord":
    ...
    def with_note(self, note: BudgetPlanningNote) -> "BudgetPlanningRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "BudgetPlanningRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "BudgetPlanningRecord":
        return cls(
            ...
            notes=tuple(BudgetPlanningNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(BudgetPlanningHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class BudgetPlanningRegister:
    def __init__(self, records: Iterable[BudgetPlanningRecord] | None = None) -> None:
        self._records: dict[str, BudgetPlanningRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: BudgetPlanningRecord) -> BudgetPlanningRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> BudgetPlanningRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> BudgetPlanningRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> BudgetPlanningRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: BudgetPlanningNote) -> BudgetPlanningRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> BudgetPlanningRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[BudgetPlanningRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[BudgetPlanningRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[BudgetPlanningRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[BudgetPlanningRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "BudgetPlanningRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(BudgetPlanningRecord.from_dict(row) for row in payload.get("records", ()))
