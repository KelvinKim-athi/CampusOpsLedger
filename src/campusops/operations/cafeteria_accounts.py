...
class CafeteriaAccountNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CafeteriaAccountNote":
        return cls(
...
@dataclass(frozen=True)
class CafeteriaAccountHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CafeteriaAccountHistoryEntry":
        return cls(
...
class CafeteriaAccountRecord:
    ...
    notes: tuple[CafeteriaAccountNote, ...] = field(default_factory=tuple)
    history: tuple[CafeteriaAccountHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "CafeteriaAccountRecord":
        status_key = clean_status(status)
        entry = CafeteriaAccountHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "CafeteriaAccountRecord":
    ...
    def with_note(self, note: CafeteriaAccountNote) -> "CafeteriaAccountRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "CafeteriaAccountRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CafeteriaAccountRecord":
        return cls(
            ...
            notes=tuple(CafeteriaAccountNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(CafeteriaAccountHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class CafeteriaAccountRegister:
    def __init__(self, records: Iterable[CafeteriaAccountRecord] | None = None) -> None:
        self._records: dict[str, CafeteriaAccountRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: CafeteriaAccountRecord) -> CafeteriaAccountRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> CafeteriaAccountRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> CafeteriaAccountRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> CafeteriaAccountRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: CafeteriaAccountNote) -> CafeteriaAccountRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> CafeteriaAccountRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[CafeteriaAccountRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[CafeteriaAccountRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[CafeteriaAccountRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[CafeteriaAccountRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "CafeteriaAccountRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(CafeteriaAccountRecord.from_dict(row) for row in payload.get("records", ()))
