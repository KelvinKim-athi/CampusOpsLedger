...
class ParkingPermitNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ParkingPermitNote":
        return cls(
...
@dataclass(frozen=True)
class ParkingPermitHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ParkingPermitHistoryEntry":
        return cls(
...
class ParkingPermitRecord:
    ...
    notes: tuple[ParkingPermitNote, ...] = field(default_factory=tuple)
    history: tuple[ParkingPermitHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ParkingPermitRecord":
        status_key = clean_status(status)
        entry = ParkingPermitHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ParkingPermitRecord":
    ...
    def with_note(self, note: ParkingPermitNote) -> "ParkingPermitRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "ParkingPermitRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ParkingPermitRecord":
        return cls(
            ...
            notes=tuple(ParkingPermitNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(ParkingPermitHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class ParkingPermitRegister:
    def __init__(self, records: Iterable[ParkingPermitRecord] | None = None) -> None:
        self._records: dict[str, ParkingPermitRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: ParkingPermitRecord) -> ParkingPermitRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> ParkingPermitRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> ParkingPermitRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> ParkingPermitRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: ParkingPermitNote) -> ParkingPermitRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> ParkingPermitRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[ParkingPermitRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[ParkingPermitRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[ParkingPermitRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[ParkingPermitRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ParkingPermitRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(ParkingPermitRecord.from_dict(row) for row in payload.get("records", ()))
