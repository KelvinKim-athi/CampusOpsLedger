...
class ExamSchedulingNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ExamSchedulingNote":
        return cls(
...
@dataclass(frozen=True)
class ExamSchedulingHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ExamSchedulingHistoryEntry":
        return cls(
...
class ExamSchedulingRecord:
    ...
    notes: tuple[ExamSchedulingNote, ...] = field(default_factory=tuple)
    history: tuple[ExamSchedulingHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ExamSchedulingRecord":
        status_key = clean_status(status)
        entry = ExamSchedulingHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ExamSchedulingRecord":
    ...
    def with_note(self, note: ExamSchedulingNote) -> "ExamSchedulingRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "ExamSchedulingRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ExamSchedulingRecord":
        return cls(
            ...
            notes=tuple(ExamSchedulingNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(ExamSchedulingHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class ExamSchedulingRegister:
    def __init__(self, records: Iterable[ExamSchedulingRecord] | None = None) -> None:
        self._records: dict[str, ExamSchedulingRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: ExamSchedulingRecord) -> ExamSchedulingRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> ExamSchedulingRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> ExamSchedulingRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> ExamSchedulingRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: ExamSchedulingNote) -> ExamSchedulingRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> ExamSchedulingRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[ExamSchedulingRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[ExamSchedulingRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[ExamSchedulingRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[ExamSchedulingRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ExamSchedulingRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(ExamSchedulingRecord.from_dict(row) for row in payload.get("records", ()))
