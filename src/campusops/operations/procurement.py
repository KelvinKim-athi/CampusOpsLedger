...
class ProcurementNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ProcurementNote":
        return cls(
...
class ProcurementHistoryEntry:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ProcurementHistoryEntry":
        return cls(
...
class ProcurementRecord:
    ...
    notes: tuple[ProcurementNote, ...] = field(default_factory=tuple)
    history: tuple[ProcurementHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ProcurementRecord":
        status_key = clean_status(status)
        entry = ProcurementHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ProcurementRecord":
    ...
    def with_note(self, note: ProcurementNote) -> "ProcurementRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "ProcurementRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ProcurementRecord":
        return cls(
            ...
            notes=tuple(ProcurementNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(ProcurementHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class ProcurementRegister:
    def __init__(self, records: Iterable[ProcurementRecord] | None = None) -> None:
        self._records: dict[str, ProcurementRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: ProcurementRecord) -> ProcurementRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> ProcurementRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> ProcurementRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> ProcurementRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: ProcurementNote) -> ProcurementRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> ProcurementRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[ProcurementRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[ProcurementRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[ProcurementRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[ProcurementRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ProcurementRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(ProcurementRecord.from_dict(row) for row in payload.get("records", ()))
