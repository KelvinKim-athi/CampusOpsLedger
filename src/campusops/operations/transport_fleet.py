...
class TransportFleetNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "TransportFleetNote":
        return cls(
...
@dataclass(frozen=True)
class TransportFleetHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "TransportFleetHistoryEntry":
        return cls(
...
class TransportFleetRecord:
    ...
    notes: tuple[TransportFleetNote, ...] = field(default_factory=tuple)
    history: tuple[TransportFleetHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "TransportFleetRecord":
        status_key = clean_status(status)
        entry = TransportFleetHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "TransportFleetRecord":
    ...
    def with_note(self, note: TransportFleetNote) -> "TransportFleetRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "TransportFleetRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "TransportFleetRecord":
        return cls(
            ...
            notes=tuple(TransportFleetNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(TransportFleetHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class TransportFleetRegister:
    def __init__(self, records: Iterable[TransportFleetRecord] | None = None) -> None:
        self._records: dict[str, TransportFleetRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: TransportFleetRecord) -> TransportFleetRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> TransportFleetRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> TransportFleetRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> TransportFleetRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: TransportFleetNote) -> TransportFleetRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> TransportFleetRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[TransportFleetRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[TransportFleetRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[TransportFleetRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[TransportFleetRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "TransportFleetRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(TransportFleetRecord.from_dict(row) for row in payload.get("records", ()))
