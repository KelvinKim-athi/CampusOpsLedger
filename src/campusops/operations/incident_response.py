...
class IncidentResponseNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "IncidentResponseNote":
        return cls(
...
@dataclass(frozen=True)
class IncidentResponseHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "IncidentResponseHistoryEntry":
        return cls(
...
class IncidentResponseRecord:
    ...
    notes: tuple[IncidentResponseNote, ...] = field(default_factory=tuple)
    history: tuple[IncidentResponseHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "IncidentResponseRecord":
        status_key = clean_status(status)
        entry = IncidentResponseHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "IncidentResponseRecord":
    ...
    def with_note(self, note: IncidentResponseNote) -> "IncidentResponseRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "IncidentResponseRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "IncidentResponseRecord":
        return cls(
            ...
            notes=tuple(IncidentResponseNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(IncidentResponseHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class IncidentResponseRegister:
    def __init__(self, records: Iterable[IncidentResponseRecord] | None = None) -> None:
        self._records: dict[str, IncidentResponseRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: IncidentResponseRecord) -> IncidentResponseRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> IncidentResponseRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> IncidentResponseRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> IncidentResponseRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: IncidentResponseNote) -> IncidentResponseRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> IncidentResponseRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[IncidentResponseRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[IncidentResponseRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[IncidentResponseRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[IncidentResponseRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "IncidentResponseRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(IncidentResponseRecord.from_dict(row) for row in payload.get("records", ()))
