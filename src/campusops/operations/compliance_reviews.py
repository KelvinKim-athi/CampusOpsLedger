...
class ComplianceReviewNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ComplianceReviewNote":
        return cls(
...
@dataclass(frozen=True)
class ComplianceReviewHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ComplianceReviewHistoryEntry":
        return cls(
...
class ComplianceReviewRecord:
    ...
    notes: tuple[ComplianceReviewNote, ...] = field(default_factory=tuple)
    history: tuple[ComplianceReviewHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ComplianceReviewRecord":
        status_key = clean_status(status)
        entry = ComplianceReviewHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ComplianceReviewRecord":
    ...
    def with_note(self, note: ComplianceReviewNote) -> "ComplianceReviewRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "ComplianceReviewRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ComplianceReviewRecord":
        return cls(
            ...
            notes=tuple(ComplianceReviewNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(ComplianceReviewHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class ComplianceReviewRegister:
    def __init__(self, records: Iterable[ComplianceReviewRecord] | None = None) -> None:
        self._records: dict[str, ComplianceReviewRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: ComplianceReviewRecord) -> ComplianceReviewRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> ComplianceReviewRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> ComplianceReviewRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> ComplianceReviewRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: ComplianceReviewNote) -> ComplianceReviewRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> ComplianceReviewRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[ComplianceReviewRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[ComplianceReviewRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[ComplianceReviewRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[ComplianceReviewRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ComplianceReviewRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(ComplianceReviewRecord.from_dict(row) for row in payload.get("records", ()))
