...
class DocumentRegistryNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DocumentRegistryNote":
        return cls(
...
@dataclass(frozen=True)
class DocumentRegistryHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DocumentRegistryHistoryEntry":
        return cls(
...
class DocumentRegistryRecord:
    ...
    notes: tuple[DocumentRegistryNote, ...] = field(default_factory=tuple)
    history: tuple[DocumentRegistryHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "DocumentRegistryRecord":
        status_key = clean_status(status)
        entry = DocumentRegistryHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "DocumentRegistryRecord":
    ...
    def with_note(self, note: DocumentRegistryNote) -> "DocumentRegistryRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "DocumentRegistryRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DocumentRegistryRecord":
        return cls(
            ...
            notes=tuple(DocumentRegistryNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(DocumentRegistryHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class DocumentRegistryRegister:
    def __init__(self, records: Iterable[DocumentRegistryRecord] | None = None) -> None:
        self._records: dict[str, DocumentRegistryRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: DocumentRegistryRecord) -> DocumentRegistryRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> DocumentRegistryRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> DocumentRegistryRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> DocumentRegistryRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: DocumentRegistryNote) -> DocumentRegistryRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> DocumentRegistryRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[DocumentRegistryRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[DocumentRegistryRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[DocumentRegistryRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[DocumentRegistryRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "DocumentRegistryRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(DocumentRegistryRecord.from_dict(row) for row in payload.get("records", ()))
