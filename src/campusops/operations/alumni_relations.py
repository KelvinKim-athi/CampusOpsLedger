...
class AlumniRelationsNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AlumniRelationsNote":
        return cls(
...
@dataclass(frozen=True)
class AlumniRelationsHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AlumniRelationsHistoryEntry":
        return cls(
...
class AlumniRelationsRecord:
    ...
    notes: tuple[AlumniRelationsNote, ...] = field(default_factory=tuple)
    history: tuple[AlumniRelationsHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "AlumniRelationsRecord":
        status_key = clean_status(status)
        entry = AlumniRelationsHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "AlumniRelationsRecord":
    ...
    def with_note(self, note: AlumniRelationsNote) -> "AlumniRelationsRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "AlumniRelationsRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AlumniRelationsRecord":
        return cls(
            ...
            notes=tuple(AlumniRelationsNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(AlumniRelationsHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class AlumniRelationsRegister:
    def __init__(self, records: Iterable[AlumniRelationsRecord] | None = None) -> None:
        self._records: dict[str, AlumniRelationsRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: AlumniRelationsRecord) -> AlumniRelationsRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> AlumniRelationsRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> AlumniRelationsRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> AlumniRelationsRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: AlumniRelationsNote) -> AlumniRelationsRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> AlumniRelationsRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[AlumniRelationsRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[AlumniRelationsRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[AlumniRelationsRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[AlumniRelationsRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "AlumniRelationsRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(AlumniRelationsRecord.from_dict(row) for row in payload.get("records", ()))
