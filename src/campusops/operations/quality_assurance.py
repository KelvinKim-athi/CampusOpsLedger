...
class QualityAssuranceNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "QualityAssuranceNote":
        return cls(
...
@dataclass(frozen=True)
class QualityAssuranceHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "QualityAssuranceHistoryEntry":
        return cls(
...
class QualityAssuranceRecord:
    ...
    notes: tuple[QualityAssuranceNote, ...] = field(default_factory=tuple)
    history: tuple[QualityAssuranceHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "QualityAssuranceRecord":
        status_key = clean_status(status)
        entry = QualityAssuranceHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "QualityAssuranceRecord":
    ...
    def with_note(self, note: QualityAssuranceNote) -> "QualityAssuranceRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "QualityAssuranceRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "QualityAssuranceRecord":
        return cls(
            ...
            notes=tuple(QualityAssuranceNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(QualityAssuranceHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class QualityAssuranceRegister:
    def __init__(self, records: Iterable[QualityAssuranceRecord] | None = None) -> None:
        self._records: dict[str, QualityAssuranceRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: QualityAssuranceRecord) -> QualityAssuranceRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> QualityAssuranceRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> QualityAssuranceRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> QualityAssuranceRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: QualityAssuranceNote) -> QualityAssuranceRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> QualityAssuranceRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[QualityAssuranceRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[QualityAssuranceRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[QualityAssuranceRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[QualityAssuranceRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "QualityAssuranceRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(QualityAssuranceRecord.from_dict(row) for row in payload.get("records", ()))
