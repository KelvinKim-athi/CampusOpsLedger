...
class ServiceDeskNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ServiceDeskNote":
        return cls(
...
class ServiceDeskHistoryEntry:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ServiceDeskHistoryEntry":
        return cls(
...
class ServiceDeskRecord:
    ...
    notes: tuple[ServiceDeskNote, ...] = field(default_factory=tuple)
    history: tuple[ServiceDeskHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ServiceDeskRecord":
        status_key = clean_status(status)
        entry = ServiceDeskHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ServiceDeskRecord":
    ...
    def with_note(self, note: ServiceDeskNote) -> "ServiceDeskRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "ServiceDeskRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ServiceDeskRecord":
        return cls(
            ...
            notes=tuple(ServiceDeskNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(ServiceDeskHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class ServiceDeskRegister:
    def __init__(self, records: Iterable[ServiceDeskRecord] | None = None) -> None:
        self._records: dict[str, ServiceDeskRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: ServiceDeskRecord) -> ServiceDeskRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> ServiceDeskRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> ServiceDeskRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> ServiceDeskRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: ServiceDeskNote) -> ServiceDeskRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> ServiceDeskRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[ServiceDeskRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[ServiceDeskRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[ServiceDeskRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[ServiceDeskRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ServiceDeskRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(ServiceDeskRecord.from_dict(row) for row in payload.get("records", ()))
