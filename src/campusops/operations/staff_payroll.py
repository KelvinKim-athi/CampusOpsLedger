...
class StaffPayrollNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StaffPayrollNote":
        return cls(
...
@dataclass(frozen=True)
class StaffPayrollHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StaffPayrollHistoryEntry":
        return cls(
...
class StaffPayrollRecord:
    ...
    notes: tuple[StaffPayrollNote, ...] = field(default_factory=tuple)
    history: tuple[StaffPayrollHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "StaffPayrollRecord":
        status_key = clean_status(status)
        entry = StaffPayrollHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "StaffPayrollRecord":
    ...
    def with_note(self, note: StaffPayrollNote) -> "StaffPayrollRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "StaffPayrollRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "StaffPayrollRecord":
        return cls(
            ...
            notes=tuple(StaffPayrollNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(StaffPayrollHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class StaffPayrollRegister:
    def __init__(self, records: Iterable[StaffPayrollRecord] | None = None) -> None:
        self._records: dict[str, StaffPayrollRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: StaffPayrollRecord) -> StaffPayrollRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> StaffPayrollRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> StaffPayrollRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> StaffPayrollRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: StaffPayrollNote) -> StaffPayrollRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> StaffPayrollRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[StaffPayrollRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[StaffPayrollRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[StaffPayrollRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[StaffPayrollRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "StaffPayrollRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(StaffPayrollRecord.from_dict(row) for row in payload.get("records", ()))
