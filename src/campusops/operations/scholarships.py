...
class ScholarshipNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ScholarshipNote":
        return cls(
...
class ScholarshipHistoryEntry:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ScholarshipHistoryEntry":
        return cls(
...
class ScholarshipRecord:
    ...
    notes: tuple[ScholarshipNote, ...] = field(default_factory=tuple)
    history: tuple[ScholarshipHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "ScholarshipRecord":
        status_key = clean_status(status)
        entry = ScholarshipHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "ScholarshipRecord":
    ...
    def with_note(self, note: ScholarshipNote) -> "ScholarshipRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "ScholarshipRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "ScholarshipRecord":
        return cls(
            ...
            notes=tuple(ScholarshipNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(ScholarshipHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class ScholarshipRegister:
    def __init__(self, records: Iterable[ScholarshipRecord] | None = None) -> None:
        self._records: dict[str, ScholarshipRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: ScholarshipRecord) -> ScholarshipRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> ScholarshipRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> ScholarshipRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> ScholarshipRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: ScholarshipNote) -> ScholarshipRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> ScholarshipRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[ScholarshipRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[ScholarshipRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[ScholarshipRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[ScholarshipRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "ScholarshipRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(ScholarshipRecord.from_dict(row) for row in payload.get("records", ()))
