...
class CourseEvaluationNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CourseEvaluationNote":
        return cls(
...
@dataclass(frozen=True)
class CourseEvaluationHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CourseEvaluationHistoryEntry":
        return cls(
...
class CourseEvaluationRecord:
    ...
    notes: tuple[CourseEvaluationNote, ...] = field(default_factory=tuple)
    history: tuple[CourseEvaluationHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "CourseEvaluationRecord":
        status_key = clean_status(status)
        entry = CourseEvaluationHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "CourseEvaluationRecord":
    ...
    def with_note(self, note: CourseEvaluationNote) -> "CourseEvaluationRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "CourseEvaluationRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "CourseEvaluationRecord":
        return cls(
            ...
            notes=tuple(CourseEvaluationNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(CourseEvaluationHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class CourseEvaluationRegister:
    def __init__(self, records: Iterable[CourseEvaluationRecord] | None = None) -> None:
        self._records: dict[str, CourseEvaluationRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: CourseEvaluationRecord) -> CourseEvaluationRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> CourseEvaluationRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> CourseEvaluationRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> CourseEvaluationRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: CourseEvaluationNote) -> CourseEvaluationRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> CourseEvaluationRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[CourseEvaluationRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[CourseEvaluationRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[CourseEvaluationRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[CourseEvaluationRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "CourseEvaluationRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(CourseEvaluationRecord.from_dict(row) for row in payload.get("records", ()))
