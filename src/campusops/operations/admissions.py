...
class AdmissionsNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AdmissionsNote":
        return cls(
...
class AdmissionsHistoryEntry:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AdmissionsHistoryEntry":
        return cls(
...
class AdmissionsRecord:
    ...
    notes: tuple[AdmissionsNote, ...] = field(default_factory=tuple)
    history: tuple[AdmissionsHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "AdmissionsRecord":
        status_key = clean_status(status)
        entry = AdmissionsHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "AdmissionsRecord":
    ...
    def with_note(self, note: AdmissionsNote) -> "AdmissionsRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "AdmissionsRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "AdmissionsRecord":
        return cls(
            ...
            notes=tuple(AdmissionsNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(AdmissionsHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class AdmissionsRegister:
    def __init__(self, records: Iterable[AdmissionsRecord] | None = None) -> None:
        self._records: dict[str, AdmissionsRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: AdmissionsRecord) -> AdmissionsRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> AdmissionsRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> AdmissionsRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> AdmissionsRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: AdmissionsNote) -> AdmissionsRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> AdmissionsRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[AdmissionsRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[AdmissionsRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[AdmissionsRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[AdmissionsRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "AdmissionsRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(AdmissionsRecord.from_dict(row) for row in payload.get("records", ()))
