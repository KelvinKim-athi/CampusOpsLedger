...
class VendorContractNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VendorContractNote":
        return cls(
...
@dataclass(frozen=True)
class VendorContractHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VendorContractHistoryEntry":
        return cls(
...
class VendorContractRecord:
    ...
    notes: tuple[VendorContractNote, ...] = field(default_factory=tuple)
    history: tuple[VendorContractHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "VendorContractRecord":
        status_key = clean_status(status)
        entry = VendorContractHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "VendorContractRecord":
    ...
    def with_note(self, note: VendorContractNote) -> "VendorContractRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "VendorContractRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VendorContractRecord":
        return cls(
            ...
            notes=tuple(VendorContractNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(VendorContractHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class VendorContractRegister:
    def __init__(self, records: Iterable[VendorContractRecord] | None = None) -> None:
        self._records: dict[str, VendorContractRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: VendorContractRecord) -> VendorContractRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> VendorContractRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> VendorContractRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> VendorContractRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: VendorContractNote) -> VendorContractRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> VendorContractRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[VendorContractRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[VendorContractRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[VendorContractRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[VendorContractRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "VendorContractRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(VendorContractRecord.from_dict(row) for row in payload.get("records", ()))
