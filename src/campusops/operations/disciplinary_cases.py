...
class DisciplinaryCaseNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DisciplinaryCaseNote":
        return cls(
...
@dataclass(frozen=True)
class DisciplinaryCaseHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DisciplinaryCaseHistoryEntry":
        return cls(
...
class DisciplinaryCaseRecord:
    ...
    notes: tuple[DisciplinaryCaseNote, ...] = field(default_factory=tuple)
    history: tuple[DisciplinaryCaseHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "DisciplinaryCaseRecord":
        status_key = clean_status(status)
        entry = DisciplinaryCaseHistoryEntry(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "DisciplinaryCaseRecord":
    ...
    def with_note(self, note: DisciplinaryCaseNote) -> "DisciplinaryCaseRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "DisciplinaryCaseRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "DisciplinaryCaseRecord":
        return cls(
            ...
            notes=tuple(DisciplinaryCaseNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(DisciplinaryCaseHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class DisciplinaryCaseRegister:
    def __init__(self, records: Iterable[DisciplinaryCaseRecord] | None = None) -> None:
        self._records: dict[str, DisciplinaryCaseRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: DisciplinaryCaseRecord) -> DisciplinaryCaseRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> DisciplinaryCaseRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> DisciplinaryCaseRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> DisciplinaryCaseRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: DisciplinaryCaseNote) -> DisciplinaryCaseRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> DisciplinaryCaseRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[DisciplinaryCaseRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[DisciplinaryCaseRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[DisciplinaryCaseRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[DisciplinaryCaseRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "DisciplinaryCaseRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(DisciplinaryCaseRecord.from_dict(row) for row in payload.get("records", ()))
