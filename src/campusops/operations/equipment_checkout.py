...
class EquipmentCheckoutNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EquipmentCheckoutNote":
        return cls(
...
@dataclass(frozen=True)
class EquipmentCheckoutHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EquipmentCheckoutHistoryEntry":
        return cls(
...
class EquipmentCheckoutRecord:
    ...
    notes: tuple[EquipmentCheckoutNote, ...] = field(default_factory=tuple)
    history: tuple[EquipmentCheckoutHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "EquipmentCheckoutRecord":
        status_key = clean_status(status)
        entry = EquipmentCheckoutHistoryEntry(
        ...
        return EquipmentCheckoutRecord(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "EquipmentCheckoutRecord":
    ...
    def with_note(self, note: EquipmentCheckoutNote) -> "EquipmentCheckoutRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "EquipmentCheckoutRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EquipmentCheckoutRecord":
        return cls(
            ...
            notes=tuple(EquipmentCheckoutNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(EquipmentCheckoutHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class EquipmentCheckoutRegister:
    def __init__(self, records: Iterable[EquipmentCheckoutRecord] | None = None) -> None:
        self._records: dict[str, EquipmentCheckoutRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: EquipmentCheckoutRecord) -> EquipmentCheckoutRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> EquipmentCheckoutRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> EquipmentCheckoutRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> EquipmentCheckoutRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: EquipmentCheckoutNote) -> EquipmentCheckoutRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> EquipmentCheckoutRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[EquipmentCheckoutRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[EquipmentCheckoutRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[EquipmentCheckoutRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[EquipmentCheckoutRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "EquipmentCheckoutRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(EquipmentCheckoutRecord.from_dict(row) for row in payload.get("records", ()))
