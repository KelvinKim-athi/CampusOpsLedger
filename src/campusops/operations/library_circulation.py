...
class LibraryCirculationNote:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "LibraryCirculationNote":
        return cls(
...
@dataclass(frozen=True)
class LibraryCirculationHistoryEntry:
    entry_id: str
    actor: str
    action: str
    message: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "LibraryCirculationHistoryEntry":
        return cls(
...
@dataclass(frozen=True)
class LibraryCirculationRecord:
    record_id: str
    subject_id: str
    title: str
    owner: str
    ...
    notes: tuple[LibraryCirculationNote, ...] = field(default_factory=tuple)
    history: tuple[LibraryCirculationHistoryEntry, ...] = field(default_factory=tuple)
    metadata: dict[str, Any] = field(default_factory=dict)
    ...
    def with_status(self, status: object, *, actor: object, message: object = "") -> "LibraryCirculationRecord":
        status_key = clean_status(status)
        entry = LibraryCirculationHistoryEntry(
        ...
        return LibraryCirculationRecord(
    ...
    def with_owner(self, owner: object, *, actor: object) -> "LibraryCirculationRecord":
    ...
    def with_note(self, note: LibraryCirculationNote) -> "LibraryCirculationRecord":
    ...
    def with_metadata(self, updates: dict[str, Any]) -> "LibraryCirculationRecord":
        metadata = deepcopy(self.metadata)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "LibraryCirculationRecord":
        return cls(
            ...
            notes=tuple(LibraryCirculationNote.from_dict(row) for row in payload.get("notes", ())),
            history=tuple(LibraryCirculationHistoryEntry.from_dict(row) for row in payload.get("history", ())),
            metadata=payload.get("metadata") or {},
        )


class LibraryCirculationRegister:
    def __init__(self, records: Iterable[LibraryCirculationRecord] | None = None) -> None:
        self._records: dict[str, LibraryCirculationRecord] = {}
        for record in records or ():
            self.add(record)

    def add(self, record: LibraryCirculationRecord) -> LibraryCirculationRecord:
        if record.record_id in self._records:
            raise ValueError(f"record already exists: {record.record_id}")
        self._records[record.record_id] = record
        return record

    def get(self, record_id: object) -> LibraryCirculationRecord:
    ...
    def assign_owner(self, record_id: object, owner: object, *, actor: object) -> LibraryCirculationRecord:
        record = self.get(record_id).with_owner(owner, actor=actor)
        self._records[record.record_id] = record
        return record

    def update_status(self, record_id: object, status: object, *, actor: object, message: object = "") -> LibraryCirculationRecord:
        record = self.get(record_id).with_status(status, actor=actor, message=message)
        self._records[record.record_id] = record
        return record

    def add_note(self, record_id: object, note: LibraryCirculationNote) -> LibraryCirculationRecord:
        record = self.get(record_id).with_note(note)
        self._records[record.record_id] = record
        return record

    def update_metadata(self, record_id: object, updates: dict[str, Any]) -> LibraryCirculationRecord:
        record = self.get(record_id).with_metadata(updates)
        self._records[record.record_id] = record
        return record

    def all_records(self) -> list[LibraryCirculationRecord]:
        return sorted(self._records.values(), key=lambda item: (item.opened_at, item.record_id))

    def by_tag(self, tag: object) -> list[LibraryCirculationRecord]:
        wanted = clean_code(tag)
        return [record for record in self.all_records() if wanted in record.tags]

    def by_owner(self, owner: object) -> list[LibraryCirculationRecord]:
        wanted = clean_code(owner)
        return [record for record in self.all_records() if record.owner == wanted]

    def priority_queue(self) -> list[LibraryCirculationRecord]:
    ...
    @classmethod
    def load_json(cls, path: str | Path) -> "LibraryCirculationRegister":
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        return cls(LibraryCirculationRecord.from_dict(row) for row in payload.get("records", ()))
